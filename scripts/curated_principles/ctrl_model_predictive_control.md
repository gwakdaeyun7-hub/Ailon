...
공간적으로 비유하면, PID 제어기는 백미러만 보고 운전하는 것이고, MPC는 전방 200미터를 내다보며 운전한다. 커브가 보이면 미리 감속하고, 교차로가 다가오면 차선을 바꾼다.
...
- **Dreamer**: Hafner et al.(2020)는 비디오 예측 기반의 세계 모델을 학습하고, 모델 안에서 미래 궤적을 시뮬레이션하며 정책을 최적화했다. 시행착오를 실제 환경이 아닌 모델 안에서 수행하여 **샘플 효율성**을 확보한다
...
- **시뮬레이션-현실 간극(Sim-to-Real Gap)**: 시뮬레이터에서 학습한 세계 모델로 현실 로봇을 제어할 때, 마찰, 공기저항, 센서 노이즈 등이 성능을 저하시키며, 이 간극을 완전히 메우기는 어렵다
...
A spatial analogy helps. A PID controller is like driving using only the rearview mirror. MPC is like driving while scanning 200 meters ahead. Seeing a curve, it decelerates in advance; approaching an intersection, it changes lanes.
...
- **Dreamer**: Hafner et al. (2020) learned a video-prediction-based world model and optimized policy by simulating future trajectories within the model. Performing trial-and-error inside the model rather than the real environment achieves **sample efficiency**
...
- **Sim-to-real gap**: When controlling real robots with world models learned in simulation, factors like friction, air resistance, and sensor noise degrade performance, and fully bridging this gap remains difficult
