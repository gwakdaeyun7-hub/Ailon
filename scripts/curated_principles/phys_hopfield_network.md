...
1970-80년대에 물리학자들은 J_ij가 무작위로 양수와 음수가 섞여 있는 "스핀 글라스(spin glass)"를 연구했다. 모든 조건을 동시에 만족시키는 배열이 존재하지 않아(좌절, frustration), 하나의 최저 에너지 상태가 아니라 무수히 많은 준안정 상태(metastable state)가 나타난다. 이 복잡한 에너지 지형이 나중에 기억과 최적화 문제의 수학적 골격이 된다.
...
패턴이 적을 때(P << 0.138N)는 각 기억이 깊고 뚜렷한 에너지 골짜기를 형성하지만, 한계에 가까워지면 골짜기가 얕아지고 서로 간섭한다. 한계를 넘으면 허위 기억(spurious state) -- 저장된 여러 패턴의 혼합물에 해당하는 에너지 극소점 -- 이 대량 출현하여 의미 있는 기억 회상이 불가능해진다. 물리학의 분석 도구가 신경망의 근본적 한계를 예측해낸 사례다.
...
이징 모델(1925)에서 출발한 에너지 함수라는 개념이 호프필드 네트워크(1982), 볼츠만 머신(1983), RBM(1986), DBN(2006)으로 이어진 것이다. 각 단계에서 원래 구조가 점차 변형되었지만, "에너지를 최소화하여 좋은 상태를 찾는다"는 핵심 아이디어는 보존되었다.
...
2024년 노벨 물리학상이 John Hopfield와 Geoffrey Hinton에게 수여되었다. 수상 사유는 "인공 신경망을 이용한 기계 학습의 기초적 발견과 발명"이었으며, 이징 모델(1925) -> 호프필드 네트워크(1982) -> 볼츠만 머신(1983) -> 심층 학습(2006~)으로 이어지는 물리학에서 AI로의 지적 전이를 공식적으로 인정한 것이다.
...
In the 1970s-80s, physicists studied "spin glasses" where J_ij values are randomly mixed positive and negative. Impossible to satisfy all conditions simultaneously (frustration), the result is not one lowest-energy state but countless metastable states. This complex energy landscape would later become the mathematical skeleton for problems of memory and optimization.
...
With few patterns (P << 0.138N), each memory forms a deep, distinct valley. Near the limit, valleys become shallow and interfere. Beyond it, spurious states -- energy minima corresponding to pattern mixtures -- proliferate, making meaningful recall impossible. Physics analysis tools predicted a fundamental neural network limitation.
...
The concept of energy functions from the Ising model (1925) passed through the Hopfield network (1982), Boltzmann machine (1983), RBM (1986), to DBN (2006). At each step the original structure was progressively transformed, but "finding good states by minimizing energy" was preserved.
