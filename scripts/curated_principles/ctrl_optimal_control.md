...
2. **정책 반복**(Policy Iteration): (1) 정책 평가(V^pi 계산)와 (2) 정책 개선(V^pi로 더 나은 행동 탐색)을 교대한다. 반복 횟수는 적지만, 매 반복마다 선형 시스템을 풀어야 한다.
...
- **AlphaGo / AlphaZero**: Silver et al.(2016, 2017)의 몬테카를로 트리 탐색(MCTS)은 게임 트리에서 벨만 방정식의 재귀적 가치 평가를 확률적 샘플링으로 근사한다. AlphaZero는 인간 기보 없이 자기 대국만으로 학습하며, "현재 가치를 미래 가치 추정으로 갱신하는" 부트스트래핑 원리를 순수하게 구현했다.
...
2. **Policy Iteration**: Alternates (1) policy evaluation (compute V^pi) and (2) policy improvement (find better actions using V^pi). Fewer iterations than value iteration, but each requires solving a linear system.
