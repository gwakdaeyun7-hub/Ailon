...
주파수 영역에서 보면, 샘플링은 원래 스펙트럼의 복제본을 f_s 간격으로 반복 배치하는 것과 같다. f_s가 충분히 크면 복제본 사이에 빈 공간이 있어 원본을 분리해낼 수 있다. f_s < 2*f_max이면 복제본들이 서로 겹치고, 겹친 부분은 원래 어느 주파수에서 온 것인지 구분할 수 없다. 한번 겹치면 되돌릴 수 없다.

일상에서 흔히 보이는 사례가 영화에서 자동차 바퀴가 거꾸로 도는 것처럼 보이는 현상이다. 카메라가 초당 24프레임으로 촬영하는데, 바퀴의 회전 주파수가 12Hz(나이퀴스트 주파수)를 넘으면 역방향으로 회전하는 것처럼 기록된다.
...
핵심 관찰은 이것이다. CNN에서 stride-2 합성곱이나 맥스 풀링은 특징 맵의 공간 해상도를 절반으로 줄이는데, 이것은 신호 처리의 다운샘플링과 정확히 같은 연산이다. 그런데 이 다운샘플링이 안티에일리어싱 없이 수행된다. 특징 맵에 높은 공간 주파수 성분이 있으면 에일리어싱이 발생하고, 입력 이미지를 1픽셀만 이동해도 출력이 크게 달라진다.
...
Zhang의 해법은 ADC 설계와 동일한 원리를 따른다. 맥스 풀링이나 스트라이드 합성곱 전에 **블러 필터**를 삽입한다. ResNet-50에서 stride-2 맥스 풀링에 3x3 블러 필터를 삽입하자, 1픽셀 이동에 대한 출력 일관성이 약 85%에서 95%로 향상되었고, ImageNet top-1 정확도도 약 0.5~1.0% 개선되었다.
...
- **NeRF 위치 인코딩**: Mildenhall et al.(2020)는 3D 좌표를 다양한 주파수의 사인-코사인 함수로 변환한다. L(주파수 대역 수)을 장면 복잡도에 맞춰야 한다는 점에서 나이퀴스트 정리의 정신과 맞닿아 있으나, 논문은 나이퀴스트를 직접 인용하지 않으며 이론적 동기는 MLP의 스펙트럼 편향 극복에 있었다
- **Super-resolution 네트워크**: 저해상도에서 고주파 디테일을 생성하는 네트워크(SRCNN, ESRGAN 등)는 나이퀴스트 율 미만으로 소실된 정보를 학습된 사전 확률로 **추정**하는 것이다. 원래 없던 정보를 만들어내는 것이지 되살리는 것이 아니라는 구분이 중요하다
...
In the frequency domain, sampling places periodic copies of the original spectrum at intervals of f_s. When f_s is large enough, copies are isolated. When f_s < 2*f_max, copies overlap irreversibly.

A familiar example: car wheels appearing to spin backward in movies. At 24 frames per second, wheel rotation above 12 Hz (the Nyquist frequency) appears reversed.

Practical systems therefore apply an **anti-aliasing filter** before sampling. This is why every ADC has a low-pass filter at its input. Losing some information is preferable to creating false information.
...
The key observation: stride-2 convolutions and max pooling halve feature map resolution -- exactly downsampling in signal processing terms. Without anti-aliasing, high spatial frequency components cause aliasing, making outputs change dramatically from a single-pixel input shift.
...
Zhang's solution: insert a **blur filter** before max pooling or strided convolution. In ResNet-50, a 3x3 blur filter before stride-2 max pooling improved shift consistency from approximately 85% to 95%, with ImageNet top-1 accuracy improving by roughly 0.5-1.0%.
...
- **NeRF positional encoding**: Mildenhall et al. (2020) transform 3D coordinates into multi-frequency sine-cosine functions. Choosing L to match scene complexity resonates with the Nyquist spirit, though the paper does not cite Nyquist directly
- **Super-resolution networks**: Networks generating high-frequency detail from low-resolution images (SRCNN, ESRGAN) are **estimating** information lost below the Nyquist rate using learned priors -- generating, not recovering, information
