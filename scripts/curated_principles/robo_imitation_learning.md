...
자율주행에서 이 문제가 특히 위험하다. 핸들을 1도 잘못 틀면, 100m 후에는 도로에서 약 1.7m 벗어난다. 이 오류가 이전에 본 적 없는 상황(갓길, 중앙선 침범)을 만들고, 그 낯선 상황에서 또 잘못된 결정을 하면 오류가 기하급수적으로 커진다.
...
- **IRL에서 RLHF, DPO로의 계보**: IRL(Ng & Russell, 2000)의 "전문가 행동에서 보상을 추론하라"는 아이디어가 출발점이다. Christiano et al.(2017)은 전문가가 직접 시연하는 대신 두 출력을 비교 판단하는 것으로 전환했고, InstructGPT(Ouyang et al., 2022)가 이를 대규모 언어모델에 적용하여 ChatGPT의 기반이 되었다. DPO(Rafailov et al., 2023)는 보상 모델 학습과 RL 루프를 하나의 지도학습 목표로 통합하여 한 단계 더 단순화했다. 모방 학습이 지도학습으로 시작했듯이, 정렬도 결국 지도학습 형태로 돌아온 것이다.
...
This problem is particularly dangerous in autonomous driving. A 1-degree steering error means drifting roughly 1.7 meters off the road after 100 meters. This error creates a situation never seen before (the shoulder, crossing the center line), and another wrong decision in that unfamiliar situation makes the error grow exponentially.
...
- **From IRL to RLHF to DPO**: IRL's (Ng & Russell, 2000) idea of "infer reward from expert behavior" was the starting point. Christiano et al. (2017) pivoted from direct expert demonstration to comparative judgments between two outputs, and InstructGPT (Ouyang et al., 2022) applied this to large language models, becoming the foundation for ChatGPT. DPO (Rafailov et al., 2023) further simplified by unifying reward model learning and the RL loop into a single supervised learning objective. Just as imitation learning began with supervised learning, alignment too has come full circle back to supervised form.
