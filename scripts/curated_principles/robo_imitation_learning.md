...
Bandura는 이를 바탕으로 사회학습 이론(Social Learning Theory, 1977)을 체계화하며, 인간 학습에는 두 가지 경로가 있다고 주장했다. 하나는 직접 시행착오를 겪는 것이고, 다른 하나는 **타인의 행동을 관찰하는 것**이다. 관찰학습은 앵무새 흉내와 다르다. 관찰자는 행동을 그대로 복사하는 것이 아니라 패턴을 추상화하여 새로운 상황에 적용할 수 있다.
...
강화학습(RL)의 수학적 기반은 Bellman의 동적 프로그래밍(1957)과 제어이론이지만, "보상으로 시행착오 학습한다"는 직관적 뿌리는 Skinner의 행동주의에 있다. 마찬가지로 모방 학습의 직관적 뿌리는 Bandura의 관찰학습이다. 직접 시행착오를 겪지 않아도, 누군가 하는 것을 보는 것만으로 배울 수 있다.
...
모방 학습(Imitation Learning)은 독립적으로 발전했지만, 사후적으로 보면 Bandura의 관찰학습과 깊은 구조적 대응을 보인다. Pomerleau(1989)가 ALVINN을 설계할 때 Bandura를 직접 참조한 증거는 없으나, "시연을 보고 배운다"는 문제 구조 자체가 동일하기 때문이다. 핵심 대응 관계는 다음과 같다.
...
행동 복제(BC)는 지도학습 하나로 끝나므로 구현이 가장 단순하지만, 전문가가 방문하지 않은 상태에 대한 대응 능력이 가장 낮다. DAgger(Ross, Gordon & Bagnell, 2011)는 이 트레이드오프를 반복적으로 좁힌다. 학습된 정책으로 환경을 실행해 새로운 상태를 수집하고, 전문가에게 올바른 행동을 라벨링 요청하여 데이터를 추가한다. 반복적 데이터 수집을 통해 BC의 O(T^2) 복합 오류를 O(T)로 줄인다 -- 제곱이 아닌 선형 누적으로 개선. 대가는 전문가를 반복 호출해야 한다는 것이다.

역강화학습(IRL, Ng & Russell, 2000)은 행동이 아니라 행동 뒤의 보상 함수를 추론하므로 일반화 능력이 가장 높지만, 계산 비용이 가장 크다. BC가 다모달 시연의 평균을 내어 잘못된 행동을 출력하는 반면, IRL은 여러 올바른 행동 모두에 높은 보상을 주는 함수를 학습할 수 있다. Ziebart et al.(2008)의 최대 엔트로피 IRL은 Jaynes의 정보이론 원칙으로 미결정 문제를 해결했고, Ho & Ermon(2016)의 GAIL은 GAN 구조로 보상 복원과 정책 학습을 통합했다.
...
- **확산 정책(Diffusion Policy)**: Chi et al.(2023)은 확산 모델을 행동 생성에 적용하여, BC의 다모달 행동 평균화 문제를 확산 과정의 다중 모드 분포 표현 능력으로 해결했다. 모방 학습의 가장 활발한 현대적 발전 방향 중 하나다.
...
Imitation Learning developed independently, but in retrospect reveals deep structural correspondence with Bandura's observational learning. There is no evidence that Pomerleau (1989) directly referenced Bandura when designing ALVINN, yet the problem structure itself -- learning from demonstrations -- is fundamentally the same. The key correspondences:
...
- **Diffusion Policy**: Chi et al. (2023) applied diffusion models to action generation, addressing BC's multimodal action averaging problem by leveraging the diffusion process's ability to represent multimodal distributions. One of the most active modern frontiers of imitation learning.
