...
1962년 후속 연구에서 시각 피질이 **엄격한 계층 구조**로 조직되어 있음을 보였다. V1(일차 시각 피질)에서 단순 세포가 에지를 감지하면, 그 신호가 복합 세포로 전달되어 위치에 무관한 에지 표현이 만들어진다. 상위 영역으로 올라갈수록 정보가 추상화된다. V2는 윤곽과 텍스처, V4는 곡선과 색상 조합, 하측두 피질(IT cortex)은 얼굴이나 손 같은 고수준 객체를 표현한다.

이 계층 구조에서 각 뉴런의 수용 영역 크기가 계층을 올라갈수록 커진다. V1 뉴런은 시야의 1~2도에만 반응하지만, IT 뉴런은 시야의 상당 부분을 덮는다. "수용 영역이 점점 커지며, 정보가 점점 추상화되는" 이 패턴이 CNN 설계의 직접적 청사진이 된다.

또한 시각 피질은 **레티노토피(retinotopic mapping)**를 유지한다. 망막에서 가까운 두 점은 피질에서도 가까이 표현된다. 이 공간적 이웃 관계 보존이 CNN이 이미지의 공간 구조를 유지하며 처리하는 설계의 생물학적 근거다. Hubel과 Wiesel은 이 발견들로 1981년 노벨 생리학 의학상을 수상했다.
...
- 단순 세포 (방향 에지 감지, 위치 민감) --> **합성곱 필터** (학습된 특징 검출기)
- 복합 세포 (위치 불변 반응) --> **풀링 레이어** (위치 변동 흡수)
- 수용 영역의 계층적 확대 --> **깊은 레이어 = 더 큰 수용 영역** (3x3 필터 3개 = 7x7 영역)
- V1 -> V2 -> V4 -> IT 계층 --> **얕은 층(에지) -> 중간 층(텍스처) -> 깊은 층(전체 객체)**
- 레티노토피 --> **특징 맵이 입력의 공간 배치를 유지**
...
Their 1962 study showed the visual cortex is organized as a **strict hierarchy**. V1 simple cells detect edges, feeding signals to complex cells for position-invariant representations. Higher areas process increasingly abstract information: V2 handles contours and textures, V4 curves and color combinations, and inferotemporal cortex (IT) represents high-level objects like faces and hands.

Critically, receptive field size grows at higher levels. V1 neurons respond to just 1-2 degrees, while IT neurons cover much of the visual field. This pattern of "growing receptive fields with increasingly abstract information" became CNN's direct blueprint.

The visual cortex also maintains **retinotopic mapping**: adjacent retinal points are represented adjacently in cortex. This spatial relationship preservation is the biological basis for CNNs maintaining spatial structure. Hubel and Wiesel received the 1981 Nobel Prize for these discoveries.
...
- Simple cells (oriented edge detection, position-sensitive) --> **convolutional filters**
- Complex cells (position-invariant response) --> **pooling layers**
- Hierarchical receptive field growth --> **deeper layers = larger receptive fields** (three 3x3 filters cover 7x7)
- V1 -> V2 -> V4 -> IT hierarchy --> **shallow layers (edges) -> middle (textures) -> deep (whole objects)**
- Retinotopic mapping --> **feature maps preserve spatial arrangement**
- Same cell type distributed across visual field --> **weight sharing**
