...
Joseph Fourier가 1807년에 제시한 핵심 주장은 이것이었다. 어떤 주기 함수든 서로 다른 주파수의 사인파와 코사인파를 적절한 비율로 더하면 재현할 수 있다. 그가 이 아이디어에 도달한 배경은 열 전도 문제였다. 금속판 위에서 열이 퍼져나가는 편미분방정식은 직접 풀기 어렵지만, 각 주파수 성분별로 분리하면 각각이 독립적인 단순한 방정식이 된다.
