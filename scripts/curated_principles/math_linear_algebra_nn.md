...
이 문제를 해결하기 위해 **직교 초기화**(orthogonal initialization)가 제안되었다. 직교 행렬의 모든 고유값 절대값은 정확히 1이므로, 반복 곱셈해도 크기가 변하지 않는다. He et al.(2015)의 Kaiming 초기화와 Glorot & Bengio(2010)의 Xavier 초기화는 가중치 행렬의 분산을 레이어 차원(fan_in, fan_out)에 맞게 조절하여, 활성화값과 그래디언트의 분산이 레이어를 거칠 때 일정하게 유지되도록 설계되었다. 이 초기화 전략들의 수학적 기반은 모두 행렬의 고유값/특이값 분포 이론이다.
...
- **LoRA (Low-Rank Adaptation)**: Hu et al.(2022)는 SVD의 저랭크 근사 원리를 모델 미세조정에 직접 적용했다. 사전 훈련된 가중치 W_0를 동결하고, 갱신량 delta_W를 두 저랭크 행렬의 곱 B * A로 파라미터화한다 (A는 r x d, B는 d x r, r << d). d = 4096, r = 8이면 원래 d^2 = 약 1,677만 개의 파라미터가 2 * d * r = 65,536개로 줄어, 약 256배 감소다. 이것이 가능한 이유는 대형 모델 미세조정 시 가중치 변화가 낮은 고유 차원에 집중되어 delta_W가 자연적으로 저랭크인 경향이 있기 때문이다(Aghajanyan et al., 2021).
...
- **GPU/TPU 하드웨어**: 행렬 곱셈은 각 원소를 독립적으로 계산할 수 있어 GPU의 병렬 구조에 완벽히 맞는다. NVIDIA의 Tensor Core는 4x4 행렬 곱셈-누적을 단일 사이클에 수행하고, Google TPU는 128x128 시스톨릭 어레이로 행렬 곱셈에 특화되었다. 하드웨어 자체가 선형대수를 물리적으로 구현한 것이다.
...
Third, **model compression**. Truncated SVD, retaining only the top k singular values, can approximate the matrix using k * (m + n + 1) parameters instead of the original m * n.
...
- **LoRA (Low-Rank Adaptation)**: Hu et al. (2022) directly applied SVD's low-rank approximation principle to model fine-tuning. Pre-trained weights W_0 are frozen and the update delta_W is parameterized as a product of two low-rank matrices B * A (A is r x d, B is d x r, r << d). With d = 4096 and r = 8, the original d^2 = roughly 16.77 million parameters shrink to 2 * d * r = 65,536 -- a roughly 256-fold reduction. This works because weight changes during large model fine-tuning concentrate in a low intrinsic dimension, meaning delta_W naturally tends to be low-rank (Aghajanyan et al., 2021).
...
- **GPU/TPU hardware**: Matrix multiplication can compute each element independently, making it a perfect match for GPU parallel architecture. NVIDIA's Tensor Cores perform 4x4 matrix multiply-accumulate in a single cycle, and Google's TPUs specialize with 128x128 systolic arrays. The hardware itself is a physical implementation of linear algebra.
