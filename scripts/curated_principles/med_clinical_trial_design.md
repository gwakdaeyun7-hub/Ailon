...
그러나 현대적 무작위 대조 시험(Randomized Controlled Trial, RCT)의 탄생은 두 가지 지적 흐름의 합류를 기다려야 했다. Ronald A. Fisher(1935)는 "The Design of Experiments"에서 무작위 배정(randomization)의 이론적 기반을 확립했다. 무작위 배정이 왜 필수적인지를 명확히 한 것이다. 알려진 교란 변수뿐 아니라 연구자가 인식하지 못한 교란 변수까지 그룹 간에 균등하게 분포시키는 유일한 방법이 무작위 배정이다.
...
A/B 테스트는 RCT의 디지털 버전이다. 구조적 대응은 거의 완벽하다.
...
이 아이디어가 80년 뒤에 다중 슬롯머신 문제(Multi-Armed Bandit, MAB)의 핵심 알고리즘으로 부활했다. Thompson Sampling은 각 선택지의 보상 분포에 대한 사후 분포에서 샘플을 뽑아 가장 높은 샘플을 가진 선택지를 고른다.
...
이 구조는 AI의 베이즈 최적화(Bayesian Optimization)와 놀라운 평행을 이룬다. 둘 다 순차적 의사결정(sequential decision-making), 불확실성 하의 탐색-활용 균형, 그리고 누적 데이터에 기반한 적응적 전략 조정이라는 같은 구조를 공유한다.
...
임상시험에서 여러 결과 변수나 하위 그룹을 동시에 검정하면 1종 오류(거짓 양성) 확률이 급격히 증가한다. 20개 독립 검정을 유의수준 0.05로 수행하면, 하나 이상에서 거짓 양성이 나올 확률은 1 - (1-0.05)^20 = 64%다.
...
This idea was resurrected 80 years later as a core algorithm for the Multi-Armed Bandit (MAB) problem. Thompson Sampling draws a sample from each arm's posterior distribution of rewards and selects the arm with the highest sample.
