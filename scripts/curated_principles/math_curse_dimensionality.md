...
---
Curse of Dimensionality (Bellman 1961) - 차원이 증가할수록 데이터가 기하급수적으로 희소해지는 현상과 그것이 학습에 미치는 근본적 한계
...
둘째, 고차원 정육면체에서 **부피가 모서리에 집중**된다. 10차원 정육면체의 중심 근처(각 축에서 0.4~0.6 사이)에 있는 부피는 전체의 0.6^10, 약 0.6%에 불과하다. 나머지 99.4%는 모서리와 꼭짓점 근처다. 데이터를 중심에서 찾으려 해도 대부분은 변두리에 있다.

셋째, **고차원 구의 부피가 0으로 수렴**한다. d차원에서 반지름 1인 구의 부피 공식은 pi^(d/2) / Gamma(d/2 + 1)인데, d가 커지면 이 값이 오히려 줄어들어 결국 0에 수렴한다. 구체적으로, 2차원 원의 넓이는 pi(약 3.14)이고, 3차원 구의 부피는 4pi/3(약 4.19)로 약간 커지지만, 이후 꾸준히 감소하여 20차원쯤이면 사실상 0에 가까워진다. 직관적으로 "구는 둥글고 넓다"는 생각이 고차원에서는 완전히 틀린다. 구 안에는 거의 아무것도 없고, 정육면체의 모서리에 부피가 몰려 있다.
...
**정규화**(regularization): L1 정규화는 불필요한 가중치를 정확히 0으로 만들어 실질적으로 특징을 제거하고, L2 정규화는 가중치를 전반적으로 작게 유지하여 고차원에서 모델이 특정 방향으로 과도하게 반응하는 것을 억제한다. 이 기법들은 모두 차원의 저주에 대한 **직접적인 수학적 대응**이다. 경험적 처방이 아니라 고차원 통계의 근본 문제를 정면으로 겨냥한다.
...
가장 유력한 답은 **다양체 가설**(manifold hypothesis)이다. 실제 데이터는 고차원 공간을 균일하게 채우지 않고, 저차원 곡면(다양체) 위에 집중되어 있다. 표면상 수십억 차원이지만 데이터가 실질적으로 변화하는 방향은 훨씬 적다. 이것이 실효 차원을 낮추어 학습을 가능하게 한다. 또한 확률적 경사하강법(SGD)이 암묵적으로 정규화 역할을 하여 과적합을 억제한다는 연구(Keskar et al., 2017)도 부분적 설명을 제공한다.
...
---EN---
Curse of Dimensionality (Bellman 1961) - The phenomenon where data becomes exponentially sparse as dimensions increase, and the fundamental limits it imposes on learning
...
Second, **volume concentrates at the edges** of high-dimensional cubes. In a 10-dimensional unit cube, the volume near the center (between 0.4 and 0.6 along each axis) is only 0.6^10, roughly 0.6% of the total. The remaining 99.4% lies near edges and corners. Even if you look for data near the center, most of it sits at the periphery.

Third, **the volume of a high-dimensional sphere converges to zero**. The volume formula for a d-dimensional unit sphere is pi^(d/2) / Gamma(d/2 + 1), and as d grows, this value actually shrinks toward zero. The intuition that "spheres are round and spacious" is completely wrong in high dimensions. The sphere contains almost nothing, while the cube's volume concentrates at its corners.
...
**Regularization**: L1 regularization drives unnecessary weights exactly to zero, effectively removing features, while L2 regularization keeps all weights small, preventing the model from overreacting to specific directions in high-dimensional space. These techniques are all **direct mathematical responses** to the curse of dimensionality -- not empirical recipes, but methods that squarely target the fundamental problems of high-dimensional statistics.
