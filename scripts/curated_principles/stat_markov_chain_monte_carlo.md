...
**깁스 샘플링**: Geman과 Geman(1984)은 이미지 복원 문제에서 다변량 분포를 다루기 위해 깁스 샘플링(Gibbs Sampling)을 제안했다. 전체 결합 분포 P(x_1, x_2, ..., x_n)에서 직접 샘플링이 어려울 때, 각 변수를 나머지 변수가 고정된 조건부 분포에서 순차적으로 샘플링한다. 변수가 10개인 문제라면, 한 번에 10차원을 탐색하는 대신 1차원씩 10번 탐색하는 것이다. 각 조건부 분포가 알려진 형태(예: 가우시안)일 때 매우 효율적이며, 사실 MH 알고리즘의 특수한 경우로 수용 확률이 항상 1이다. 1990년대 BUGS(Bayesian inference Using Gibbs Sampling) 소프트웨어는 베이즈 통계의 실용화를 이끌었다.

**해밀턴 몬테카를로(HMC)**: 일반적인 MH 알고리즘은 무작위 걸음(random walk)으로 탐색하므로 고차원 공간에서 비효율적이다. 변수가 100개인 문제에서 무작위로 방향을 고르면, 대부분의 방향이 확률이 낮은 영역을 향하므로 수용률이 급격히 떨어진다. Duane, Kennedy, Pendleton, Roweth(1987)는 물리학의 **해밀턴 역학**을 샘플링에 도입하여 이 문제를 해결했다. 각 파라미터 위치에 가상의 운동량(momentum) 변수를 추가하고, -log P(x)를 위치 에너지로 해석한다. 해밀턴 방정식에 따라 위치와 운동량이 함께 진화하면, 입자는 높은 확률 영역의 등고선을 따라 효율적으로 이동한다. HMC가 무작위 걸음 MH와 근본적으로 다른 점은 **그래디언트 정보를 활용**한다는 것이다. -log P(x)의 기울기가 이동 방향을 안내하므로, 고차원에서도 높은 수용률과 빠른 탐색이 가능하다. Hoffman과 Gelman(2014)의 NUTS(No-U-Turn Sampler)는 HMC의 궤적 길이를 자동으로 결정하여 실용성을 크게 높였다. Stan과 PyMC에서 기본 샘플러로 채택되어 현대 베이즈 통계의 표준 도구가 되었다.
...
**Gibbs Sampling**: Geman and Geman (1984) proposed Gibbs Sampling for image restoration problems. When direct sampling from a multivariate distribution P(x_1, x_2, ..., x_n) is difficult, each variable is sequentially sampled from its conditional distribution with all others held fixed. For a 10-variable problem, instead of exploring 10 dimensions at once, you explore 1 dimension at a time, 10 times. It is very efficient when each conditional has a known form (e.g., Gaussian) and is actually a special case of MH where the acceptance probability is always 1. In the 1990s, BUGS (Bayesian inference Using Gibbs Sampling) software drove the practical adoption of Bayesian statistics.

**Hamiltonian Monte Carlo (HMC)**: Standard MH algorithms explore via random walks and become inefficient in high dimensions. In a 100-variable problem, choosing a random direction means most directions point toward low-probability regions, so acceptance rates plummet. Duane, Kennedy, Pendleton, and Roweth (1987) solved this by introducing **Hamiltonian mechanics** into sampling. Auxiliary momentum variables are added for each parameter position, and -log P(x) is interpreted as potential energy. As position and momentum evolve together according to Hamilton's equations, the particle efficiently travels along contours of high probability. HMC is fundamentally different from random-walk MH in that it **leverages gradient information**. The gradient of -log P(x) guides movement direction, enabling high acceptance rates and fast exploration even in high dimensions. Hoffman and Gelman's (2014) NUTS (No-U-Turn Sampler) automatically determines HMC trajectory length, greatly improving practicality. Adopted as the default sampler in Stan and PyMC, it has become the standard tool of modern Bayesian statistics.
