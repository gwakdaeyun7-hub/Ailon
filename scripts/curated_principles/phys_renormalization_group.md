...
4. 이 과정을 반복할 때마다 격자의 해상도가 낮아진다. 구체적으로, 100x100 격자(10,000개 스핀)에서 2x2 블록을 하나로 묶으면 50x50(2,500개)이 된다. 다시 묶으면 25x25(625개), 또 묶으면 12x12(약 144개)가 된다. 격자점은 10,000에서 144로 줄었지만, 전체 시스템의 거시적 행동 -- 예를 들어 자석이 되는 온도 -- 은 보존된다. 3x3 블록 기준이면 100x100이 33x33, 다시 11x11이 된다
...
연속적인 스케일 변화에서 RG 변환은 베타 함수(beta function)로 기술된다. 베타 함수는 "스케일을 조금 바꾸면 결합 상수가 얼마나 변하는가"를 알려주는 변화율이다.

beta(g) = dg/d(ln s)

g는 결합 상수(입자 간 상호작용의 세기를 나타내는 숫자), s는 스케일 변화 인자다. beta(g) = 0인 점이 고정점이다. 비유하면, 베타 함수는 강물의 유속과 같다. 유속이 0인 곳(고정점)에서 물은 멈추고, 양수인 곳에서 물은 한 방향으로 흐르며, 음수인 곳에서 반대로 흐른다.

고정점 근처에서 결합 상수가 어떤 방향으로 흐르는지를 선형 분석하면 임계 지수(critical exponent)가 결정된다. 예를 들어 자화 M은 임계 온도 T_c 근처에서 M ~ (T_c - T)^beta로 거동하는데, 이 지수 beta(여기서는 임계 지수로, 위의 베타 함수와는 다른 기호)는 물질의 미시적 세부에 무관하고 공간 차원과 대칭성에만 의존한다. 이것이 보편성의 수학적 표현이다.
...
관련 연산자(relevant operator)는 RG 변환을 거듭할수록 커지는 방향이다. 비유하면, 줌아웃해도 여전히 보이는 것들이다. 이들이 임계 거동을 결정하며, 임계 지수를 지배한다. 예를 들어 온도 차이 (T - T_c)는 관련 연산자다. 조대화를 반복하면 이 차이가 증폭되어 시스템이 고정점에서 멀어진다.

비관련 연산자(irrelevant operator)는 반복할수록 줄어드는 방향이다. 줌아웃하면 사라지는 세부사항에 해당한다. 원자의 구체적 배열 방식이나 격자의 세부 형태가 여기에 속한다. 위성사진에서 줌아웃하면 개별 건물의 색깔은 사라지지만 "도시"라는 구조는 남는 것처럼, 비관련 연산자가 줄어들기 때문에 서로 다른 물질이 동일한 고정점으로 수렴할 수 있다. 보편성의 수학적 메커니즘이 바로 비관련 연산자의 소멸이다.

관련/비관련의 구분은 고정점에서의 RG 변환을 선형화한 행렬(stability matrix)의 고유값으로 결정된다. 고유값이 1보다 크면 관련(줌아웃해도 남는 것), 1보다 작으면 비관련(줌아웃하면 사라지는 것)이다. 이 분류가 보편성 클래스(universality class)를 정의한다. 같은 보편성 클래스에 속하는 시스템은 관련 연산자의 수와 종류가 동일하다.
...
한편, 이 유비에 대한 비판적 분석도 존재한다. Beny(2013)는 RG 변환의 정보 이론적 구조가 특정 조건에서만 심층 학습과 대응됨을 지적했고, Koch-Janusz와 Ringel(2018)은 신경망으로 최적 RG 변환을 학습할 수 있지만 그 결과가 기존 물리학의 RG 흐름과 정량적으로 일치하는 경우는 제한적임을 보였다. 이 연구들은 유비의 깊이가 표면적 구조 유사성 이상으로 확장되기 어려울 수 있음을 시사한다.
...
4. Each repetition lowers the lattice resolution. Concretely, grouping 2x2 blocks in a 100x100 lattice (10,000 spins) yields 50x50 (2,500). Another round gives 25x25 (625), then 12x12 (roughly 144). The lattice points drop from 10,000 to 144, yet the system's macroscopic behavior -- for instance, the temperature at which it becomes magnetic -- is preserved. With 3x3 blocks, a 100x100 lattice becomes 33x33, then 11x11
...
For continuous scale changes, the RG transformation is described by the beta function. The beta function tells you "how much does the coupling constant change when you shift the scale slightly" -- a rate of change.

beta(g) = dg/d(ln s)

Here g is the coupling constant (a number representing the strength of interaction between particles) and s is the scale factor. Points where beta(g) = 0 are fixed points. By analogy, the beta function is like a river's current. Where the current is zero (fixed point), the water stands still; where positive, it flows one direction; where negative, the opposite direction.

Linear analysis of how coupling constants flow near the fixed point determines the critical exponents. For example, magnetization M near the critical temperature T_c behaves as M ~ (T_c - T)^beta, where beta (here a critical exponent, distinct from the beta function) is independent of microscopic details and depends only on spatial dimension and symmetry. This is the mathematical expression of universality.
...
Relevant operators grow under repeated RG transformations. By analogy, they are the things that remain visible even after zooming out. They determine critical behavior and govern critical exponents. For example, the temperature difference (T - T_c) is a relevant operator. Repeated coarse-graining amplifies this difference, driving the system away from the fixed point.

Irrelevant operators shrink under repeated transformations. They are the details that vanish when you zoom out. Specific atomic arrangements or detailed lattice geometry fall in this category. Just as individual building colors disappear in a satellite image when zooming out but the structure called "city" remains, irrelevant operators shrink, which is why different materials can converge to the same fixed point. The mathematical mechanism of universality is precisely the vanishing of irrelevant operators.

The relevant/irrelevant classification is determined by the eigenvalues of the stability matrix -- the linearized RG transformation at the fixed point. Eigenvalues greater than 1 indicate relevant (persisting through zoom-out); less than 1 indicate irrelevant (vanishing through zoom-out). This classification defines universality classes. Systems in the same universality class share the same number and type of relevant operators.
...
Critical analyses of this analogy also exist. Beny (2013) pointed out that the information-theoretic structure of RG transformations maps onto deep learning only under specific conditions, and Koch-Janusz and Ringel (2018) showed that while neural networks can learn optimal RG transformations, the results match traditional physics RG flows quantitatively only in limited cases. These studies suggest that the analogy may be difficult to extend beyond surface-level structural similarity.
