...
---
Cybernetics (Wiener 1948) - 기계와 생물에 공통하는 피드백 기반 통신·제어의 통합 이론

## 피드백 루프의 본질

에어컨 온도 조절기를 떠올려 보자. 목표 온도를 25도로 설정하면, 조절기는 현재 실내 온도를 측정한다. 28도라면 그 **차이(오차)** 3도가 냉방을 가동하게 만든다. 실내가 25도에 도달하면 냉방이 멈춘다. 이것이 가장 단순한 피드백 루프다. 결과가 다시 원인으로 되돌아가 행동을 조정하는 순환 고리. 핵심 구조는 세 가지로 이루어진다. 도달하려는 **목표**(설정값), 현재 상태를 읽는 **감지기**(센서), 그리고 오차를 줄이는 방향으로 행동하는 **조절기**(액추에이터)다.

이 구조는 기계에만 있는 것이 아니다. 인간의 체온 조절도 같은 원리다. 체온이 올라가면 땀을 흘려 식히고, 내려가면 근육을 떨어 열을 낸다. 동공도 밝으면 수축하고 어두우면 확장한다. 20세기 초 생리학자 Walter Cannon은 이런 생물체의 자기조절 능력을 **항상성**(homeostasis)이라 불렀다. 생물과 기계가 공유하는 이 되먹임 구조가, 새로운 학문의 출발점이 된다.

## 위너와 사이버네틱스의 탄생

2차 세계대전 중, 수학자 Norbert Wiener는 공학자 Julian Bigelow와 함께 대공포 예측 시스템을 연구하고 있었다. 문제는 이것이었다. 적기가 현재 보이는 위치에 포탄을 쏘면 이미 늦는다. 적기의 **미래 위치를 예측**하고, 실제 비행 궤적과 예측의 차이를 지속적으로 보정해야 한다. 이 작업은 본질적으로 피드백 루프였다. 예측, 측정, 오차 계산, 보정의 반복이다.
...
1948년 Wiener는 이 통찰을 "Cybernetics: Or Control and Communication in the Animal and the Machine"이라는 책으로 출간했다. '사이버네틱스'라는 이름은 그리스어 **kybernetes**(조타수)에서 따왔다. 배의 조타수가 바람과 파도에 따라 키를 끊임없이 조정해 목적지로 향하는 모습이, 피드백 기반 제어의 본질을 담고 있었다.
...
1946년부터 1953년까지, 뉴욕에서 **메이시 회의**(Macy Conferences)가 열렸다. 이 회의는 학문 역사에서 매우 독특한 자리였다. 수학자 Wiener, 신경과학자 Warren McCulloch, 정보이론의 Claude Shannon, 인류학자 Margaret Mead, 심리학자 Kurt Lewin 등이 한자리에 모였다. 주제는 '피드백과 순환 인과성'이었다.

이 지적 생태계에서 여러 씨앗이 동시에 자랐다. McCulloch와 Walter Pitts는 뉴런을 논리 게이트로 모델링한 논문(1943)을 발표했고, 이것은 인공신경망의 수학적 기초가 되었다. Shannon은 정보를 비트로 측정하는 이론(1948)을 제시했다. 이 아이디어들은 서로 영향을 주고받으며 성장했다. 피드백으로 자기를 조절하는 시스템이라는 사이버네틱스의 핵심 관념이, 기계가 학습하고 적응할 수 있다는 발상의 토양이 된 것이다.

1956년 다트머스 회의(Dartmouth Conference)에서 "인공지능"이라는 이름이 공식적으로 탄생했을 때, 그 사상적 토양의 상당 부분은 사이버네틱스가 일구어 놓은 것이었다. 기계도 피드백을 통해 목표를 추구할 수 있다는 생각 없이는, 기계에 지능을 부여한다는 발상 자체가 나오기 어려웠을 것이다.

## AI로의 직접 연결

사이버네틱스의 피드백 원리는 현대 AI의 여러 핵심 구조에 직접적 영향을 미쳤다.

**퍼셉트론(Perceptron)**: Rosenblatt(1958)의 퍼셉트론은 사이버네틱 전통에서 직접 태어났다. 감각 입력을 가중합하여 분류하고, 분류 결과가 틀리면 오차 신호를 이용해 가중치를 조정한다. 목표(정답)와 출력의 차이를 되먹여 행동(가중치)을 교정하는 구조는 피드백 루프 그 자체다.

**강화학습(Reinforcement Learning)**: 에이전트가 환경에서 행동하고, **보상**이라는 피드백 신호를 받아 행동을 교정한다. 높은 보상을 목표로 삼고, 현재 보상과의 차이(시간차 오차)를 줄이는 방향으로 정책을 갱신한다. 이 구조는 사이버네틱 피드백 루프의 직접적 구현이다. Wiener가 말한 "목표 지향적 행동의 수학적 구조"가 알고리즘으로 실현된 것이다.

**순환 신경망(RNN)**: 네트워크의 출력이 다음 시간 단계의 입력으로 되먹임되는 구조다. 이전 상태의 정보가 순환하며 시간에 걸친 의존성을 학습한다. 이것은 피드백 루프의 신경망 구현이며, 시계열 예측, 언어 모델링 등에서 핵심 역할을 했다.

**역전파(Backpropagation)**: 신경망의 출력 오차를 네트워크 역방향으로 전파하여 각 가중치의 기여도를 계산하고 갱신한다. 출력과 목표의 차이(손실)가 전체 시스템을 조정하는 신호가 된다는 점에서, 사이버네틱 피드백의 심화된 형태다. 다만 역전파는 생물학적으로 존재하는 메커니즘이 아니며, 사이버네틱스의 피드백 개념을 수학적으로 확장한 것이라는 점을 구분해야 한다.

## 한계와 쇠퇴, 그리고 부활

1960년대에 접어들며 사이버네틱스는 서서히 주류에서 밀려났다. 가장 큰 이유는 범위가 **지나치게 넓었다**는 것이다. 기계, 생물, 사회, 경제까지 모든 것을 피드백으로 설명하려다 보니, 정작 구체적인 수학적 도구와 알고리즘 개발에서는 깊이가 부족해졌다. Marvin Minsky와 John McCarthy가 이끈 상징적 AI(Symbolic AI)가 논리와 표상에 기반한 구체적 프로그램을 제시하며 주류를 장악했다.

하지만 사이버네틱스의 핵심 아이디어는 사라지지 않았다. 피드백을 통한 자기조절과 적응이라는 원리는 제어이론으로 정교화되었고, 시스템이론으로 확장되었다. 그리고 궁극적으로 강화학습, 순환 신경망, 적응 제어 등 현대 AI의 근간에서 살아 숨 쉰다.
...
---EN---
Cybernetics (Wiener 1948) - A unified theory of feedback-based communication and control common to both machines and living organisms
...
The core lesson of cybernetics is this: stable self-regulating systems rely on negative feedback. The cycle of sensing the gap between goal and current state, then acting to close that gap, is the source of stability. This principle reappears in engineering servomechanisms, in biological homeostasis, and later in AI learning algorithms.

## The Macy Conferences and Interdisciplinary Impact

From 1946 to 1953, the **Macy Conferences** were held in New York. These gatherings occupied a uniquely remarkable place in intellectual history. Mathematician Wiener, neuroscientist Warren McCulloch, information theorist Claude Shannon, anthropologist Margaret Mead, and psychologist Kurt Lewin all sat at the same table. The theme was "feedback and circular causality."

Multiple seeds grew simultaneously in this intellectual ecosystem. McCulloch and Walter Pitts published their paper (1943) modeling neurons as logic gates, laying the mathematical foundation for artificial neural networks. Shannon introduced the theory of measuring information in bits (1948). These ideas grew by cross-pollinating one another. The cybernetic core concept of self-regulating systems through feedback became the soil from which the idea that machines could learn and adapt would grow.

When "artificial intelligence" was officially named at the 1956 Dartmouth Conference, a substantial portion of its intellectual groundwork had been cultivated by cybernetics. Without the idea that machines could pursue goals through feedback, the very notion of endowing machines with intelligence would have been far more difficult to conceive.
...
**Reinforcement Learning**: An agent acts in an environment and receives a **reward** as a feedback signal to correct its behavior. It aims for high reward and updates its policy in the direction that reduces the difference between current and target reward (temporal difference error). This structure is a direct implementation of the cybernetic feedback loop -- Wiener's "mathematical structure of goal-directed behavior" realized as an algorithm.

**Recurrent Neural Networks (RNN)**: The network's output feeds back as input for the next time step. Information from previous states circulates, enabling learning of dependencies across time. This is a neural network implementation of the feedback loop, playing a central role in time series prediction and language modeling.

**Backpropagation**: Output error propagates backward through the network to compute each weight's contribution and update it. In that the difference between output and target (loss) becomes the signal that adjusts the entire system, this is a deepened form of cybernetic feedback.

## Decline and Revival

As the 1960s began, cybernetics gradually lost its mainstream position. The primary reason was that its scope was **too broad**. Attempting to explain everything -- machines, organisms, society, economics -- through feedback led to insufficient depth in developing concrete mathematical tools and algorithms. Symbolic AI, led by Marvin Minsky and John McCarthy, seized the mainstream by offering concrete programs based on logic and representation.

But the core ideas of cybernetics never vanished. The principle of self-regulation and adaptation through feedback was refined into control theory and expanded into systems theory. And ultimately, it lives and breathes at the foundation of modern AI -- in reinforcement learning, recurrent neural networks, and adaptive control. In every process where today's robots sense their environment, adjust their actions, and pursue their goals, the feedback loop that Wiener drew lies at the blueprint's core.
