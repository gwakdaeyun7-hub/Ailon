...
- 경쟁적이다 -- 여러 자극이 제한된 처리 자원을 놓고 서로 경쟁한다(Desimone & Duncan, 1995)
...
Q(Query)는 "내가 찾고 있는 것", K(Key)는 "내가 가진 것의 레이블", V(Value)는 "실제 내용"이다. Q * K^T로 유사도를 계산하고, sqrt(d_k)로 나누어 스케일을 보정한 뒤, softmax로 확률 분포로 변환하여 V에 가중합을 적용한다. d_k = 64일 때 나누지 않으면 내적값이 64 규모가 되어 softmax 출력이 거의 원-핫이 되고 기울기가 소실된다.

다중 헤드 어텐션(multi-head attention)은 동일한 입력에 서로 다른 선형 변환을 적용하여 여러 관점에서 동시에 관계를 포착한다. 실제로 8개 헤드를 사용하면, 어떤 헤드는 문법적 관계를, 다른 헤드는 의미적 관계를 학습하는 것이 관찰된다.
...
- 방향성: 뇌의 주의는 상향식 + 하향식이 상호작용한다. Transformer 주의는 데이터에서 학습된 패턴으로, 하향식 제어에 해당하는 구조가 부재한다
- 공간성 vs 토큰 관계: 뇌의 시각적 주의는 망막 좌표계의 공간적 위치에 초점을 맞춘다. Transformer 주의는 토큰 간 추상적 유사도를 계산하며, 공간 개념 자체가 없다
- 용량 제한: 뇌의 주의는 동시에 4-7개 항목으로 제한된다. Transformer 주의는 모든 토큰 쌍을 동시에 계산하며, 제한이 없는 것이 오히려 O(N^2) 비용이라는 문제를 만든다
- 억제 방식: 뇌의 주의는 억제성 뉴런을 통해 무관 정보를 적극 차단한다. softmax는 상대적으로 낮은 가중치를 부여할 뿐, 명시적 억제가 아니다
...
- **Self-attention**: 시퀀스 내 모든 토큰이 서로를 참조하는 것은 인지적 주의에 대응물이 없다. 뇌에서 "모든 감각 입력이 다른 모든 입력과의 관련성을 동시에 계산하는" 과정은 일어나지 않는다
- **효율적 주의 연구**: FlashAttention(Dao et al., 2022) 등은 순수한 하드웨어 최적화 문제이며, 인지과학과 무관한 공학적 발전이다
- **Sparse attention**: 긴 시퀀스에서 일부 토큰 쌍만 선택하는 것은, 역설적으로 뇌의 용량 제한적 주의에 가까워지지만, 동기는 연산 비용 절감이지 생물학적 모방이 아니다
...
- Capacity-limited -- only 4-7 items simultaneously (Cowan, 2001)
- Selective -- enhances relevant and **actively suppresses** irrelevant information
- Competitive -- stimuli compete for limited resources (Desimone & Duncan, 1995)
...
Q (Query) is "what I'm looking for," K (Key) is "the label of what I have," V (Value) is "the actual content." Q * K^T computes similarity, division by sqrt(d_k) corrects scaling, and softmax converts scores to a probability distribution applied as weights to V. Without the sqrt(d_k) division (e.g., d_k = 64), dot products reach magnitude 64, making softmax output near one-hot and causing gradient vanishing.

Multi-head attention applies different linear transformations to capture relationships from multiple perspectives simultaneously. With 8 heads in practice, some learn syntactic relationships while others learn semantic ones.
...
- Directionality: Brain attention involves bottom-up and top-down interaction. Transformer attention has no structure for top-down control
- Spatial vs. token relationships: Brain visual attention focuses on spatial locations. Transformer attention computes abstract token similarities with no spatial concept
- Capacity limits: Brain attention is limited to 4-7 items. Transformer attention computes all pairs simultaneously, and having no limit creates the O(N^2) cost problem
- Suppression: Brain attention actively blocks via inhibitory neurons. Softmax merely assigns low weights -- not explicit suppression
...
- **Self-attention**: Every token referencing every other token has no counterpart in cognitive attention
- **Efficient attention research**: FlashAttention (Dao et al., 2022) and similar work are pure hardware optimization, unrelated to cognitive science
- **Sparse attention**: Selecting only some token pairs paradoxically moves closer to capacity-limited brain attention, but the motivation is cost reduction, not biological emulation
