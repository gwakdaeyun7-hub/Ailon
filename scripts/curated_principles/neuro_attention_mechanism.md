...
**필터 이론** -- Broadbent(1958)는 주의를 정보 처리 초기 단계의 병목 필터로 모델링했다. 처리 용량의 한계 때문에 물리적 특성(목소리 톤, 방향)에 기반하여 하나의 채널만 통과시키고, 나머지는 차단한다. Shannon의 정보 이론에 영향받은 모델이다.
...
5. sqrt(d_k)로 나누는 것은 이 스케일 문제를 보정한다. d_k = 64일 때, 두 벡터의 내적 기댓값은 0이 아니라 64 규모가 된다. 소프트맥스에 64 규모의 수를 넣으면 출력이 거의 원-핫(한 곳에 몰림)이 되어 기울기가 소실된다. sqrt(64) = 8로 나누면 내적이 적절한 범위로 돌아온다
...
**Filter theory** -- Broadbent (1958) modeled attention as a bottleneck filter at early processing stages. Due to limited processing capacity, only one channel passes based on physical properties (voice tone, direction), while the rest is blocked. This model was influenced by Shannon's information theory.

**Attenuation theory** -- Treisman (1964) revised this, arguing that unattended channel information is not fully blocked but weakened (attenuated). Survival-relevant words like one's own name or "Fire!" have low activation thresholds, allowing them to reach consciousness even through weakened signals.
...
5. Dividing by sqrt(d_k) corrects this scaling issue. When d_k = 64, the expected magnitude of the dot product of two vectors is not 0 but on the order of 64. Feeding numbers of magnitude 64 into softmax produces a near one-hot output (all weight concentrated on one position), causing gradients to vanish. Dividing by sqrt(64) = 8 brings the dot products back to a suitable range
