...
핵심 메커니즘은 다음과 같다. 시그모이드의 가중치 w를 극단적으로 키우면 계단 함수에 수렴하고, 두 계단 함수의 차이로 특정 구간에서만 값이 1인 직사각형 펄스를 만들 수 있다. 이런 펄스들의 높이와 위치를 조절하여 합산하면, 어떤 연속 함수든 계단식으로 쌓아 올릴 수 있다. 펄스를 좁게, 많이 쌓을수록 근사가 정밀해진다.
...
존재 정리는 "그러한 대상이 존재한다"만 말하고, 구성 정리는 "이렇게 만들 수 있다"를 알려준다. 최적의 가중치가 파라미터 공간 어딘가에 존재한다는 것과 경사하강법이 그 가중치를 실제로 찾아낼 수 있다는 것은 완전히 다른 문제다. 보편 근사 정리가 침묵하는 네 가지 핵심 질문이 있다.

- **뉴런이 몇 개 필요한가?** 정리는 유한 개면 충분하다고만 한다. 입력 차원 d, 정밀도 epsilon일 때 필요한 뉴런 수가 (1/epsilon)^d에 비례할 수 있어, d=10이면 현실적으로 불가능한 숫자다. Barron(1993)은 특정 조건하에서 O(1/sqrt(N)) 바운드를 보였지만 일반적이지 않다
- **SGD가 최적 가중치를 찾을 수 있는가?** 손실 함수가 비볼록(non-convex)이므로 지역 최솟값에 갇힐 수 있다. 2층 ReLU 네트워크의 최적화조차 NP-hard다(Blum & Rivest, 1992)
- **학습 데이터 바깥에서 작동하는가?** 정리는 일반화(generalization)에 대해 아무것도 말하지 않는다. PAC 학습 이론, VC 차원 등 별도의 이론 틀이 필요하다
- **계산 비용이 감당 가능한가?** 이론적 가능성과 실용적 가능성은 별개다. 100차원 입력의 함수 근사에 우주의 원자 수보다 많은 뉴런이 필요할 수도 있다

이 네 질문은 보편 근사 정리 이후 신경망 이론의 핵심 과제가 되었다.
...
- **과파라미터화와 이중 하강**: Belkin et al.(2019)가 보고한 이중 하강 현상 -- 파라미터가 데이터보다 훨씬 많은 모델이 오히려 잘 일반화 -- 은 보편 근사 정리의 범위를 완전히 벗어난다. 이 현상은 고전 편향-분산 트레이드오프에도 모순되며, 별도의 이론 틀이 필요하다
- **Transformer의 보편 근사 성질**: Yun et al.(2020)은 Transformer도 보편 근사기임을 증명했지만, 실제 성공 이유는 self-attention의 귀납적 편향이 자연어의 장거리 의존성 구조와 맞아떨어진다는 점에 있다
...
An existence theorem says only "such an object exists"; a construction theorem tells you "how to build it." The existence of optimal weights somewhere in parameter space is an entirely different matter from whether gradient descent can actually find them. The theorem is silent on four critical questions:

- **How many neurons are needed?** The theorem says only that a finite number suffices. With input dimension d, the required count can scale as (1/epsilon)^d -- practically impossible for d=10. Barron (1993) showed O(1/sqrt(N)) bounds under specific conditions, but these are not general
- **Can SGD find the optimal weights?** The loss landscape is non-convex, so the optimizer may get trapped in local minima. Even 2-layer ReLU optimization is NP-hard (Blum & Rivest, 1992)
- **Does it generalize beyond training data?** The theorem says nothing about generalization. Separate frameworks (PAC learning, VC dimension) are required
- **Is the computational cost feasible?** Approximating a 100-dimensional function might require more neurons than atoms in the universe

These four questions became the core challenges for neural network theory after the theorem.
...
- **Overparameterization and double descent**: Belkin et al.'s (2019) double descent -- models with far more parameters than data generalizing well -- lies entirely outside the theorem's scope, also contradicting classical bias-variance tradeoff
- **Universal approximation of Transformers**: Yun et al. (2020) proved Transformers are also universal approximators, but their actual success comes from self-attention's inductive bias aligning with natural language's long-range dependency structure
