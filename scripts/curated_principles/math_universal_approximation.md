...
- **Mixture of Experts(MoE)**: 전문가를 늘려 용량을 확장하되, 게이팅 함수가 입력마다 소수만 활성화하여 계산 비용을 억제한다. 보편 근사 정리가 보장하는 "충분한 뉴런"의 용량 문제를, 전체를 동시에 쓰지 않는 건축적 설계로 해결한 사례다.
...
- **Mixture of Experts (MoE)**: Scales capacity by adding experts while a gating function activates only a few per input to contain cost. An architectural solution to the "sufficient neurons" capacity problem guaranteed by the theorem, without using all of them simultaneously.
