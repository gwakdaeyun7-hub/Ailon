...
존재 정리는 "그러한 대상이 존재한다"만 말하고, 구성 정리는 "이렇게 만들 수 있다"를 알려준다. 최적의 가중치가 파라미터 공간 어딘가에 존재한다는 것과 경사하강법이 그 가중치를 실제로 찾아낼 수 있다는 것은 완전히 다른 문제다. 보편 근사 정리가 침묵하는 네 가지 핵심 질문이 있다. (1) **얼마나 많은 뉴런이 필요한가?** 정리는 "충분히 많은" 뉴런이 존재한다고만 말하며, 그 수가 입력 차원에 대해 다항적인지 기하급수적인지 침묵한다. (2) **최적 가중치를 어떻게 찾는가?** 가중치가 존재한다는 것과 경사하강법으로 도달할 수 있다는 것은 별개다. 비볼록 손실 지형에서의 최적화 보장은 정리의 범위 밖이다. (3) **학습 데이터 밖에서도 작동하는가?** 근사는 훈련 영역에서의 정합성만 보장한다. 새로운 입력에 대한 일반화는 전혀 다른 이론적 문제다. (4) **계산적으로 효율적인가?** 이론적으로 가능한 것과 실용적 시간 안에 학습 가능한 것은 다르다. NP-hard 문제를 포함하여, 근사 가능하지만 학습 불가능한 경우가 존재한다.
...
An existence theorem says only "such an object exists"; a construction theorem tells you "how to build it." The existence of optimal weights somewhere in parameter space is an entirely different matter from whether gradient descent can actually find them. The theorem is silent on four critical questions. (1) **How many neurons are needed?** The theorem says only that "sufficiently many" neurons exist, remaining silent on whether the number grows polynomially or exponentially with input dimension. (2) **How do we find the optimal weights?** The existence of weights and whether gradient descent can reach them are separate matters. Optimization guarantees in non-convex loss landscapes lie outside the theorem's scope. (3) **Does it work beyond the training data?** Approximation guarantees only consistency within the training domain. Generalization to new inputs is an entirely different theoretical question. (4) **Is it computationally efficient?** What is theoretically possible and what is learnable in practical time are different things. Problems exist that are theoretically approximable but practically unlearnable, including NP-hard optimization problems.
