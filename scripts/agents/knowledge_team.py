...
def discipline_expert_node(state: KnowledgeState) -> dict:
    """특정 학문 분야의 핵심 원리를 생성 (LLM 1회)"""
    info = state["discipline_info"]
    print(f"\n📚 [DisciplineExpertAgent] {info['name']} 분야 핵심 원리 생성 중...")

    llm = get_llm(temperature=0.8, max_tokens=4096)

    prompt = f"""당신은 {info['name']} 분야의 세계적인 전문가입니다.
AI를 공부하는 사람들에게 도움이 되는 {info['name']}의 기본적이지만 핵심적인 원리 3개를 생성해주세요.
...
- explanation: 상세 설명 2-3문장 (한국어, 비전공자도 이해 가능하게)
- realWorldExample: 실생활 예시 1-2문장 (한국어)
- applicationIdeas: AI에 적용할 수 있는 아이디어 3개 (한국어, 배열)
...
[
  {{
    ...
    "applicationIdeas": ["AI 적용 1", "AI 적용 2", "AI 적용 3"]
  }}
]"""
    ...
    try:
        ...
        principles = [{
            ...
            "description": f"{info['name']}의 핵심 개념입니다.",
            "explanation": f"{info['focus']}에 관한 기본 원리입니다.",
            "realWorldExample": "다양한 분야에서 활용됩니다.",
...
def ai_relevance_node(state: KnowledgeState) -> dict:
    """생성된 원리에 AI 관련성과 타 학문 연결고리를 추가 (LLM 1회)"""
    ...
    prompt = f"""당신은 AI와 학제간 연구의 전문가입니다.
...
각 원리에 대해 다음을 추가해주세요:
- aiRelevance: 이 원리가 AI 개발/연구/응용에 왜 중요한지 2-3문장 (한국어)
- crossDisciplineLinks: 이 원리와 연결되는 다른 학문 분야 2-3개와 연결 이유 (한국어, 배열)
...
[
  {{
    ...
    "realWorldExample": "...",
    "applicationIdeas": [...],
    "aiRelevance": "AI와의 관련성...",
    "crossDisciplineLinks": ["수학: 연결 이유", "뇌과학: 연결 이유"],
    ...
    try:
        ...
        enriched = principles
        for p in enriched:
            p["aiRelevance"] = info.get("ai_connection", "")
...
def quality_reviewer_node(state: KnowledgeState) -> dict:
    """정확성, 실용성, 이해도를 검증하고 최종 다듬기 (LLM 1회)"""
    ...
    prompt = f"""당신은 교육 콘텐츠 품질 검수 전문가입니다.
다음 {info['name']} 원리들을 검수하고 최종 버전을 작성해주세요.

검수 기준:
1. 정확성: 학문적으로 정확한가?
2. 이해도: 비전공자 대학생이 이해할 수 있는가?
3. 실용성: AI를 공부하는 사람에게 실질적 도움이 되는가?
4. 흥미도: 읽고 싶어지는 내용인가?

다음을 수행하세요:
- 설명이 너무 전문적이면 쉽게 수정
- 예시가 약하면 더 좋은 예시로 교체
- AI 관련성이 피상적이면 구체적으로 보강
- 가장 품질이 높은 원리 1개를 "best"로 선택

반드시 다음 JSON 형식으로만 응답하세요:
{{
  "principles": [검수 완료된 원리 배열],
  "best_index": 0
}}
