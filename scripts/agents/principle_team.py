...
{{
  ...
  "application": {{
    ...
    "problem": "AI가 부딪힌 구체적 난제 한 줄 (30~50자, 예: '수백만 파라미터를 동시에 조정하는 것은 불가능에 가까웠다')",
    "problem_en": "One-line AI challenge statement (e.g., 'Tuning millions of parameters simultaneously seemed impossible')",
    "body": "이 원리가 위 난제를 어떻게 해결하는지 (80~120자, 2~3줄)",
    "body_en": "How this principle solves the above challenge (2-3 sentences)",
  ...
  "deepDive": {{
    "originalProblem": "이 원리가 처음 필요했던 학문적 맥락 (100~150자, '어떤 학문에서, 어떤 현상을 설명하기 위해 등장했는가?' 형식, 핵심 인물/년도 포함)",
    "originalProblem_en": "Academic context where this principle first emerged (2-3 sentences, must include discipline, phenomenon, key figures and years)",
    "bridge": "영감의 다리 — 누가, 어떤 통찰로 이 원리를 AI에 가져왔는가? (150~200자, 반드시 '보존된 것'과 '변형된 것'을 구분하여 서술)",
    "bridge_en": "Bridge of inspiration — Who brought this principle to AI, and with what insight? (3-5 sentences, must distinguish what was preserved vs. what was transformed)",
    "coreIntuition": "핵심 직관 — 이 원리가 하는 일을 한 문장으로 + 수식/알고리즘의 직관적 해석 (100~150자)",
    "coreIntuition_en": "Core intuition — One-sentence summary of what this principle does + intuitive interpretation of the formula/algorithm (2-3 sentences)",
    "formula": "핵심 수식이나 알고리즘 (LaTeX 형태, 없으면 빈 문자열)",
    "formula_en": "Core formula or algorithm in English (LaTeX, or empty string if none)",
    "limits": "한계와 열린 질문 — 이 비유가 깨지는 구체적 지점 + 아직 해결되지 않은 문제 + 연구 방향 (100~150자)",
    "limits_en": "Limits and open questions — Where the analogy breaks down, unresolved problems, and research directions (2-3 sentences)"
  }},
...
- application.problem: "AI가 부딪힌 구체적 벽"을 생생하게 묘사 (추상적 표현 금지, 구체적 수치나 상황 사용)
- application.body: problem에서 제시한 난제의 해법을 설명 (원리가 어떻게 해결하는지)
- deepDive.bridge (가장 중요한 섹션): "보존된 것"과 "변형된 것"을 명시적으로 구분할 것 (예: "~의 원리는 보존되었으나, ~는 ~로 변형되었다")
- deepDive.limits: 구체적 한계를 서술할 것 (추상적 "아직 갈 길이 멀다" 금지, 구체적으로 어떤 조건에서 비유가 깨지는지, 어떤 문제가 미해결인지 명시)
...
4. **영감의 다리(bridge)**: deepDive.bridge에서 "보존된 것"과 "변형된 것"이 명시적으로 구분되어 있는가? 구분이 사실에 기반하는가? (예: 수학적 구조가 보존되었다면 실제로 동일 수식을 사용하는지, 변형되었다면 구체적으로 무엇이 달라졌는지)
5. **한계의 구체성(limits)**: deepDive.limits가 구체적 한계를 서술하는가? 추상적 표현("아직 갈 길이 멀다") 없이 비유가 깨지는 구체적 조건과 미해결 문제를 명시하는가?
6. **팩트 오류**: 날짜, 인물, 논문명 등 구체적 사실에 오류가 없는가?
7. **과장 여부**: 연결의 강도가 과장되지 않았는가?
