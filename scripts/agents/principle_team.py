...
{{
  ...
  "foundation": {{
    ...
    "analogy": "핵심 메커니즘(과정)을 비유한 한 줄 (30~60자). 결과 비유 금지",
  ...
  "integration": {{
    ...
    "impact": "검증 가능한 프로젝트/모델/사례 기반 임팩트 (30~50자). 추상 표현 금지",
  ...
  "deepDive": {{
    ...
    "bridge": "원래 학문→AI 대응 관계를 명시적 매핑(X→Y)으로 최소 3쌍 제시 + 보존된 구조 vs 변형/생략된 부분 구분 (150~200자)",
    "bridge_en": "Explicit mapping (X→Y) of at least 3 pairs from original discipline to AI + preserved vs. transformed/omitted (3-5 sentences)",
    "coreIntuition": "핵심 수식의 완전한 작동 규칙: ① 각 변수 의미 ② 조건별 분기(언제 무조건 수용/언제 확률적) ③ 파라미터 변화→행동 변화 ④ 수렴 조건. analogy 수준이면 실격 (100~180자)",
    "coreIntuition_en": "Full decision rule: ① variable meanings ② conditional branches (when always accept vs probabilistic) ③ parameter sensitivity ④ convergence conditions. DISQUALIFIED if analogy-level (3-4 sentences)",
    "formula": "핵심 수식 1개(LaTeX). 수학/물리/정보이론/통계이면 필수. 단일 줄, \\begin/\\end 금지, \\frac 1단 중첩까지",
    "formula_en": "One LaTeX formula. REQUIRED for math/physics/info-theory/stats. Single-line, no environments, \\frac max 1 nesting",
    "limits": "AI 적용 시 구체적 한계 — AI-specific 문제 필수 (100~150자)",
...
- originalProblem: 논문/저서명 1개+, 인물+년도. foundation의 "근본 질문"과 이어지는 학술적 맥락
- bridge (가장 중요): 원래 학문에서 AI로 건너올 때 **구체적 대응 관계**를 명시 + 보존/변형 구분
  - 필수 체크: ① 원래 학문의 핵심 변수/개념 최소 3개를 AI 대응물과 명시적으로 짝지을 것 (X→Y 형태) ② 보존된 것: 어떤 수학적 구조/논리가 그대로 살아남았는지 ③ 변형/생략된 것: 원래 분야에서는 중요하지만 AI에서 버린 것은 무엇이고 왜 버렸는지
...
- coreIntuition: 핵심 수식/알고리즘의 **완전한 작동 규칙**을 서술. analogy와 같은 일상 비유이면 실격
  - 필수 체크: ① 각 변수가 AI에서 무엇을 의미하는지 ② **조건별 분기**: 수식이 적용되는 조건과 적용되지 않는 조건 (예: "ΔE≤0이면 무조건 수용, ΔE>0일 때만 e^(-ΔE/T)>U로 확률 판단") ③ **파라미터 감수성**: 각 변수 변화가 동작을 어떻게 바꾸는지 (예: "T↑→수용확률↑→광역탐색, ΔE 작을수록→수용 쉬움") ④ **수렴 조건**: 어떤 설정이면 수렴 확률이 높아지는지 (예: "T0↑, cooling_rate→1, 반복횟수↑이면 전역 최적해 수렴 확률↑")
  - beginner: 용어 1~2개 + 풀어쓴 설명 (예: "적합도(fitness)가 높은 해를 더 많이 복제하여...")
  - intermediate: 용어 3~4개 (예: "N차원 파라미터 공간에서 손실 곡면의...")
  - advanced: 수학적 엄밀성 (예: "확률 분포 p(x)의 KL divergence를 최소화하는...")
- formula: 수학/물리/정보이론/통계이면 빈 문자열 금지
- limits: "더 연구 필요" 수준 금지. AI-specific 한계만 (distribution shift, non-convexity, adversarial robustness 등)
...
- difficulty: beginner=수학 불필요, intermediate=기초 미적분/선형대수, advanced=대학원 수준
- JSON만 출력 (추가 설명 없이)
...
10. **영감의 다리(bridge)**: 원래 학문→AI의 명시적 대응 매핑(X→Y)이 최소 3쌍 있는가? "보존된 핵심 구조"와 "변형/생략된 부분"이 구체적 예시와 함께 명시적으로 구분되어 있는가? (수학/물리: 수학적 구조 보존/변형, 생물학/심리학: 추상화된 원리/버려진 세부사항 — 분야에 맞는 프레임을 쓰고 있는가?) 매핑 없이 추상적 서술만 하면 deepDiveDepth ≤ 0.5
11. **핵심 직관(coreIntuition)**: 수식의 **완전한 작동 규칙**이 서술되어 있는가? ① 각 변수의 AI 의미 ② 조건별 분기 (예: "ΔE≤0→무조건 수용, ΔE>0→확률적 판단") ③ 파라미터 감수성 (변수 변화→동작 변화) ④ 수렴 조건이 있는가? 이 4가지 중 2개 이하만 있으면 deepDiveDepth ≤ 0.5. foundation.analogy와 동일한 일상 비유 수준이면 deepDiveDepth ≤ 0.4로 강하게 감점
