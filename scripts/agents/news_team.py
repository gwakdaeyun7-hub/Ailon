...
def _summarize_batch(batch: list[dict], batch_idx: int, translate: bool = True) -> list[dict] | None:
    ...
    if translate:
        ...
        title_rule = (
            ...
            "  - 글자 수 제한 없음. 축약하지 말 것\n"
            "  - ★★★ '...' 및 '?' 규칙 (가장 중요 — 위반 시 전체 출력 재생성):\n"
            "    [적극 사용] 기사 톤에 어울리면 비율 제한 없이 자유롭게 '...'와 '?'를 사용\n"
            "    [어울리는 경우] 여운·궁금증·예고·충격·반전·불확실성·열거 암시·호기심 유발·전망·의문\n"
            "      ✅ 'OpenAI, 차세대 모델 힌트...출시 임박?' / 'AI가 일자리를 대체할까?'\n"
            "      ✅ 'Anthropic, 무려 60조 가치...' / 'GPU 전쟁, 어디까지 갈까...'\n"
            "    [절대 금지] '...'를 쉼표 대용 구분자로 쓰는 것. 확정 사실 서술어 뒤 '...'로 부가정보 연결 금지\n"
            "      금지 서술어 (이 외 모든 확정 서술어도 포함): 공개/출시/발표/인수/도입/개발/선언/철회/체결/중단/제기/투자/가동/확대/보류\n"
            "      ❌ 'NVIDIA, 모델 공개... 처리량 5배' → ✅ 'NVIDIA, 모델 공개, 처리량 5배 향상'\n"
            "      ❌ 'Google, Wiz 인수... 역대 최대' → ✅ 'Google, Wiz 320억 달러 인수'\n"
            "    [자가 검증] '...'를 쉼표(,)로 바꿔 보라. 의미가 같으면 → 쉼표 사용 (구분자 오용)"
        )
        en_fields_rule = (
