  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FAFAFA' }} edges={['top']}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={{ marginHorizontal: 16, marginBottom: 16, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 20, ...cardShadow }}>
          <Text style={{ color: '#757575', fontSize: 12, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 14 }}>{t('notification.title')}</Text>
          {notifPermission === 'denied' ? (
            <View style={{ alignItems: 'center', gap: 8, paddingVertical: 8 }}>
              <Text style={{ color: '#BDBDBD', fontSize: 13, textAlign: 'center' }}>{t('notification.denied')}</Text>
              <Pressable
                onPress={() => {
                  if (Platform.OS === 'ios') Linking.openURL('app-settings:');
                  else Linking.openSettings();
                }}
                style={{ backgroundColor: '#E53935', paddingHorizontal: 20, paddingVertical: 10, borderRadius: 12 }}
              >
            ...
            <View style={{ alignItems: 'center', gap: 8, paddingVertical: 8 }}>
              <Text style={{ color: '#BDBDBD', fontSize: 13, textAlign: 'center' }}>{t('notification.enable_desc')}</Text>
              <Pressable
                onPress={async () => {
            ...
            <View style={{ gap: 12 }}>
              {([
                ...
                <View key={key} style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                  ...
                  <Text style={{ flex: 1, color: '#212121', fontSize: 15, fontWeight: '600' }}>{label}</Text>
                  <Switch
                    value={settings[key]}
                    onValueChange={(v) => updateSetting(key, v)}
                    trackColor={{ false: '#E0E0E0', true: '#FFCCBC' }}
                    thumbColor={settings[key] ? '#E53935' : '#BDBDBD'}
                  />
                </View>
              ))}
            </View>
