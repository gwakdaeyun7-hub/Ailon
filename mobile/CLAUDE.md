...
- **Article Card**: display_title, one_line, sections (소제목+내용 2-4개), why_important, background, tags, glossary, "Read Original" button (openArticle → expo-web-browser In-App Browser). 요약 모달: F-Minimal 디자인 (헤더: X닫기(좌)+날짜(우), 드래그 핸들 없음. 메타 행: 소스 뱃지+읽기시간(Clock, 우측 marginLeft auto), 배경↔본문 구분선, 세리프 제목, One Line 16pt(장식 없음), 소제목+내용 sections 15pt(배경 없음), 본문↔왜중요해요 구분선, 세리프 소제목 textSecondary, 태그 pill, 원문 버튼 텍스트 pill 스타일: surface 배경, borderRadius 14, 우측 하단 alignSelf flex-end). 기존 Firestore key_points 데이터는 폴백 렌더링 지원
