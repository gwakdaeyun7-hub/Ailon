...
function CommentItem({
  ...
  return (
    <View style={{ flexDirection: 'row', gap: 10, paddingVertical: 10, paddingLeft: isReply ? 44 : 0, borderBottomWidth: 1, borderBottomColor: '#F5F5F5' }}>
      {isReply && (
        <CornerDownRight size={14} color="#BDBDBD" style={{ marginTop: 4 }} />
      )}
      <View style={{ width: isReply ? 28 : 34, height: isReply ? 28 : 34, borderRadius: isReply ? 14 : 17, backgroundColor: isReply ? '#FFF3E0' : '#FFEBEE', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        <Text style={{ color: isReply ? '#F57C00' : '#E53935', fontSize: isReply ? 10 : 12, fontWeight: '700' }}>{initials}</Text>
      </View>
      <View style={{ flex: 1 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 3 }}>
          <Text style={{ color: '#212121', fontSize: isReply ? 12 : 13, fontWeight: '700' }}>{comment.authorName}</Text>
          <Text style={{ color: '#BDBDBD', fontSize: 11 }}>{date}</Text>
        </View>
        <Text style={{ color: '#424242', fontSize: isReply ? 13 : 14, lineHeight: 20 }}>{comment.text}</Text>
        {!isReply && onReply && (
          <Pressable
