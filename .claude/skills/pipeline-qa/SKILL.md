...
| verifier 파싱 방어 | 에러 없이 완료 | `verifier JSON 파싱 2회 연속 실패` 로그 → 기본 fail 결과 사용 → retry_reseed 경로 확인 | Major |

**방어 로직 정상 작동 확인 기준:**
- `content_generator: 필수 키 누락` 로그 후 → `[retry_reseed] 시드 교체` 로그가 이어지면 정상
- `verifier JSON 파싱 2회 연속 실패` 로그 후 → `[retry_reseed]` 로그가 이어지면 정상
...
- `_VERIFY_PROMPT` (line ~519): 검증 프롬프트 — 이해도 검증 항목 추가
- `_CONTENT_PROMPT` 내 deepDive 스키마 (line ~388-399): bridge/limits 구체성 강화
- `content_generator()` (line ~436): 생성 로직 — 필수 키 누락 시 `content=None` 반환 → retry_reseed
- `verifier()` (line ~557): 검증 로직 + confidence 임계값 — JSON 파싱 2회 실패 시 기본 fail(verified=False, confidence=0.0) 반환
...
| bridge 보존/변형 미구분 | prompt-engineer | _CONTENT_PROMPT 내 deepDive 스키마 강화 (line ~388-399) |
...
| 학문스낵 생성 프롬프트 | principle_team.py | `_CONTENT_PROMPT` | ~333-432 |
| 학문스낵 Deep Dive 섹션 | principle_team.py | `_CONTENT_PROMPT` 내 인라인 JSON 스키마 | ~388-399 |
| 학문스낵 검증 프롬프트 | principle_team.py | `_VERIFY_PROMPT` | ~519 |
| 학문스낵 생성 로직 | principle_team.py | `content_generator()` | ~436 |
| 학문스낵 검증 로직 | principle_team.py | `verifier()` | ~557 |
