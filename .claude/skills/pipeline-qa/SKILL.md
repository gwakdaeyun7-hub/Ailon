...
**수정 대상 파일:** `scripts/agents/news_team.py` — `_CLASSIFY_PROMPT` (line ~1349)
...
- `EMBED_DEDUP_THRESHOLD` (line ~1037): 임베딩 유사도 임계값 (현재 0.92)
- `_normalize_title()` (line ~1058): title=None 방어 포함
- `_DEDUP_STOPWORDS` (line ~1070): 변별력 없는 토큰 set
- `_extract_key_tokens()` (line ~1073): text=None 방어 포함
- `_extract_product_versions()` (line ~1115): title=None 방어 포함
- L4 고유명사 가드: one_line≥0.65 + 양쪽 제목에 고유명사(영어)가 있으면 1개+ 공유 필수 (line ~1223)
- L5 조건: `names_overlap >= 3 and nums_overlap >= 1` (line ~1244)
- L7 `overlap_ratio >= 0.30` (line ~1271)
...
**수정 대상 파일:** `scripts/agents/news_team.py`
- `_RANK_PROMPT` (line ~1381): 랭킹 기준 프롬프트
- `_rank_category()` (line ~1404): token_budget, ctx_len 임계값
- 컨텍스트 축소 기준: line ~1418+
- token_budget: line ~1441
...
| 분류 프롬프트 | news_team.py | `_CLASSIFY_PROMPT` | ~1349+ |
| 분류 배치 | news_team.py | `_classify_batch()` | ~1510 |
| 의심 키워드 | news_team.py | `_SUSPECT_KEYWORDS` | ~1717+ |
| 중복 임계값 | news_team.py | `DEDUP_THRESHOLD`, `EMBED_DEDUP_THRESHOLD` | ~1036-1037 |
| 중복 stopwords | news_team.py | `_DEDUP_STOPWORDS` | ~1070 |
| 중복 L4 가드 | news_team.py | `_deduplicate_candidates()` L4 고유명사 가드 | ~1223+ |
| 중복 L5 조건 | news_team.py | `_deduplicate_candidates()` L5 | ~1244 |
| 중복 L7 조건 | news_team.py | `_deduplicate_candidates()` L7 | ~1271 |
| 랭킹 프롬프트 | news_team.py | `_RANK_PROMPT` | ~1381+ |
| 랭킹 로직 | news_team.py | `_rank_category()` | ~1404+ |
| 랭킹 token_budget | news_team.py | `_rank_category()` 내 | ~1441 |
| 컨텍스트 축소 | news_team.py | `_rank_category()` 내 ctx_len | ~1418+ |
