...
6. **요약 품질**: sections 0-1개 기사, one_line 누락 기사
...
9. **sections 구조 검증**: 각 기사의 sections가 2-4개 섹션 배열인지, 각 섹션에 subtitle과 content가 있는지 확인. subtitle이 "개요", "결론", "기타", "주요 내용", "요약" 등 일반적 단어가 아닌 구체적 명사구(3-5단어)인지 확인. content가 서술체(~했다, ~됐다) 3-5문장인지 확인
...
로그 패턴:
- `[요약 통계] sections 부족(0-1개): {n}건, why_important 누락: {n}건, background 누락: {n}건`
- `[요약 말투] 위반 없음` 또는 `요약 말투 위반 {n}건 감지` + 개별 위반 상세
- `[QA] 요약 품질 이슈 {n}건 감지` 또는 `[QA] 요약 품질 이슈 없음`
- `요약 상세: 전체 기사 요약` — 접기 섹션: 기사별 one_line, sections, why_important, background 전문

**데이터 소스 우선순위 (요약 품질 검증용):**

요약 품질 검증은 기사별 실제 데이터(one_line, sections, why_important, background)가 필요합니다. 데이터 소스를 다음 우선순위로 확보:

| 우선순위 | 소스 | 조건 | 검증 범위 |
|----------|------|------|-----------|
| 1 | `요약 상세: 전체 기사 요약` 접기 섹션 | CI 로그에 포함됨 | 전체 기사 |
| 2 | Firestore 직접 조회 | `scripts/` 디렉토리에 Firebase credentials 접근 가능 | 샘플 5건 (하이라이트 3 + 카테고리 Top 2) |
| 3 | 없음 — 미검증 | 위 소스 모두 불가 | 검증 불가 → **Unverified** 판정 |

**대체 경로 2 (Firestore 직접 조회) 실행 방법:**
```python
# scripts/ 디렉토리에서 실행
...
# 하이라이트 기사 조회
highlights = db.collection("articles").where("date", "==", today).where("is_highlight", "==", True).limit(3).get()
# 카테고리별 Top 기사 조회
top_articles = db.collection("articles").where("date", "==", today).order_by("score", direction=firestore.Query.DESCENDING).limit(5).get()

for doc in highlights + top_articles:
    d = doc.to_dict()
    print(f"--- {d.get('title_ko', d.get('title',''))}")
    print(f"one_line: {d.get('one_line','')}")
    print(f"sections: {json.dumps(d.get('sections',[]), ensure_ascii=False, indent=2)}")
    print(f"why_important: {d.get('why_important','')}")
    print(f"background: {d.get('background','')}")
```
- Firebase credentials가 `.env` 파일 또는 환경 변수에 있으면 실행 가능
- credentials가 없으면 사용자에게 "CI 로그의 접기 섹션 또는 Firebase credentials가 필요합니다" 안내 후 **Unverified** 판정

**미검증(Unverified) 처리:**
- 접기 섹션도 없고 Firestore 조회도 불가하면, 1.10.2(말투)~1.10.4(EN/KO) 중 로그 통계로 검증 가능한 항목만 수행
- 1.10.3(콘텐츠 품질)과 subtitle-content 정합성은 **Unverified**로 표기
- 리포트의 요약 품질 판정을 "Pass"가 아닌 **"Unverified — 데이터 미확보"**로 명시
...
| sections 보유율 (2개+) | 90-100% | 0-1개 기사 5건+ → 요약 프롬프트 문제 또는 스크래핑 실패 | Major |
| sections 구조 완결성 | 각 섹션에 subtitle + content 존재 | subtitle 또는 content 누락 → 파싱 오류 | Major |
| subtitle 품질 | 구체적 명사구 (3-5단어) | "개요", "결론", "기타", "주요 내용", "요약" 등 일반적/모호한 단어 | Major |
| content 문장 수 | 3-5문장 (마침표 기준) | 1-2문장 → 내용 부족, 6문장+ → 과다 | Minor |
| why_important 보유율 | 95-100% | 누락 다수 → 프롬프트 미준수 | Minor |
| background 보유율 | 90-100% | 누락 다수 → 프롬프트 미준수 | Minor |
| EN 필드 쌍 (one_line_en, sections_en, why_important_en, background_en) | 모든 기사 | 누락 → EN 사용자 요약 불가 | Major |
| 레거시 key_points 폴백 | Firestore 기존 key_points 데이터 → 모바일에서 정상 렌더링 | key_points 데이터가 있는 기사에서 모바일 렌더링 실패 | Major |
...
| sections content | 서술체 | ~했다, ~됐다, ~밝혔다 | ~했어요, ~이에요, ~임, ~됨 (개조식도 금지) |
| why_important | 해요체 | ~이에요, ~해요, ~있어요, ~돼요, ~거예요 | ~했다, ~됐다, ~됩니다, ~입니다 |
| background | 서술체 | ~했다, ~됐다, ~있었다 | ~했어요, ~이에요, ~돼요 |
| glossary desc | 해요체 | ~이에요, ~해요 | ~이다, ~됩니다, ~했다 |

**파이프라인 자동 감지**: `요약 말투 위반 {n}건 감지` 로그로 기본 패턴 자동 검출 (`_SEC_BAD_RE` 정규식으로 sections content 경어체 위반 탐지). 파이프라인이 감지하지 못하는 미묘한 위반(예: sections content가 "~임"처럼 개조식이지만 정규식 미매칭)은 `요약 상세` 접기 섹션에서 수동 확인.

**기사 단위 말투 검증 (필수):**
- `요약 상세: 전체 기사 요약` 접기 섹션의 각 기사 요약을 확인
- one_line이 "~했어요"로 끝나면 → 말투 위반 (서술체여야 함)
- sections content가 "~했어요", "~이에요" (경어체) 또는 "~임", "~됨" (개조식)으로 끝나면 → 말투 위반 (서술체여야 함)
- sections subtitle이 문장형 종결어미를 포함하면 → 말투 위반 (명사구여야 함)
...
| 검사 항목 | 좋은 예 | 나쁜 예 | 심각도 |
|-----------|---------|---------|--------|
| **subtitle 구체성** | "2조 파라미터 아키텍처 혁신", "API 가격 및 멀티모달 지원" | "개요", "결론", "기타", "주요 내용", "요약", "정리" | Major |
| **subtitle 길이** | 3-5단어 명사구 | 1단어("소개") 또는 문장형("모델이 출시되었다") | Minor |
| **content 구체적 데이터** | "GPT-6는 총 2조 개의 파라미터를 사용하며, GPT-5 대비 4배 규모로 확장됐다." | "성능이 크게 향상됐다." — 수치 없음 | Major |
| **content 문장 수** | 3-5문장 (마침표 기준) | 1-2문장 (내용 부족) 또는 6문장+ (과다) | Minor |
| **content 서술체** | "~했다", "~됐다", "~밝혔다" | "~임", "~됨" (개조식), "~했어요" (경어체) | Major |
| **one_line 비중복** | one_line에 없는 새로운 정보 | one_line의 같은 사실을 다른 말로 반복 | Major |
| **섹션 간 비중복** | 각 섹션이 서로 다른 정보 전달 | 같은 내용을 다른 섹션에서 반복 서술 | Minor |
| **금지 패턴 미사용** | 고유명사·숫자·스펙 포함 | "관심을 받고 있다", "주목받고 있다", "전망이다" — 막연한 평가 | Major |
...
| **one_line/sections 비중복** | 새로운 관점(영향·결과)만 서술 | one_line이나 sections 내용을 다시 언급 | Major |
...
**기사 단위 콘텐츠 검증 절차:**
1. 데이터 확보: 데이터 소스 우선순위(1.10 상단)에 따라 기사별 요약 데이터를 확보
   - 접기 섹션 있음 → **하이라이트 3개 + 카테고리 Top 2개** (총 5건) 확인
   - 접기 섹션 없음 → Firestore 조회로 동일 5건 샘플 확보
   - 모두 불가 → 콘텐츠 품질 + subtitle-content 정합성 = **Unverified**로 표기, 4번으로 건너뜀
2. 각 기사의 one_line → sections (subtitle+content) → why_important → background 순으로 검토
3. **subtitle-content 정합성 검증** (위 "subtitle-content 정합성 검증" 기준에 따라 각 섹션 검증)
4. 다음 유형의 이슈를 식별:

| 이슈 유형 | 정의 | 예시 |
|-----------|------|------|
| **내용 중복** | one_line↔sections↔why_important 간 같은 정보 반복 | one_line "GPT-5 출시했다" + sections content "GPT-5가 출시됐다" |
| **추상적 sections** | 숫자·스펙 없이 막연한 서술 | "성능이 크게 향상됐다", "주목받고 있다" |
| **일반적 subtitle** | "개요", "결론", "요약" 등 모호한 소제목 | subtitle이 "주요 내용"으로만 구성 |
| **빈 why_important** | 구체적 대상·변화 없이 평가만 | "업계에 큰 영향", "경쟁 치열" |
| **일반적 background** | 특정 사건·맥락 없이 범용 서술 | "AI 기술이 빠르게 발전 중이다" |
| **환각(Hallucination)** | 기사 제목에서 추론 가능한 범위를 넘어선 정보 | 기사 제목 "Meta, 새 모델 발표"에 대해 sections에 구체적 파라미터 수를 포함 (확인 불가) |

**주의: 환각 판별 한계** — pipeline-qa는 로그만으로 환각을 완전히 감지할 수 없음 (원문 본문 미포함). 제목과 명백히 불일치하는 정보만 환각 의심으로 표기. 미묘한 환각은 앱에서 사용자 피드백으로 감지.

**소제목(subtitle) 품질 판별 기준:**
- 금지 단어: "개요", "결론", "기타", "주요 내용", "요약", "정리", "핵심 사항", "Overview", "Conclusion", "Summary", "Key Details"
- 좋은 subtitle: "2조 파라미터 아키텍처 혁신", "API 가격 및 멀티모달 지원", "오픈소스 라이선스 전환", "성능 벤치마크 결과"
- 나쁜 subtitle: "주요 내용", "결론", "첫 번째 포인트", "요약"
- subtitle이 빈 문자열이면 Critical

**subtitle-content 정합성 검증 (★ 기사 단위 필수):**

subtitle이 금지 단어를 피했더라도, content의 실제 주제를 대표하지 않으면 품질 이슈입니다. 각 기사의 섹션마다 다음을 검증:

**검증 방법 — 키워드 교차 확인:**
1. subtitle에서 핵심 키워드를 추출 (고유명사, 수치, 기술 용어 — 조사/접속사 제외)
2. 추출된 키워드 중 **2개 이상**이 해당 섹션의 content에 등장하는지 확인
3. 키워드가 1개 이하만 매칭되면 정합성 의심 → 아래 불일치 유형으로 분류

| 불일치 유형 | 정의 | 예시 | 심각도 |
|-------------|------|------|--------|
| **주제 불일치** | subtitle의 주제와 content의 주제가 다름 | subtitle "가격 정책 변화" → content는 성능 벤치마크만 서술 | Major |
| **과도한 포괄** | subtitle이 너무 넓어서 content의 구체적 주제를 반영 못 함 | subtitle "기술 동향" → content는 특정 모델의 아키텍처 상세 | Major |
| **키워드 미반영** | subtitle의 고유명사/수치가 content에 없음 | subtitle "2조 파라미터 아키텍처" → content에 "2조", "파라미터" 미등장 | Major |
| **역방향 누락** | content의 핵심 주제가 subtitle에 전혀 반영되지 않음 | content는 "오픈소스 라이선스 전환"이 핵심 → subtitle "모델 업데이트" | Minor |

**정합성 검증 예시:**
```
기사: "OpenAI, GPT-5 출시"
  섹션 1:
    subtitle: "2조 파라미터 아키텍처 혁신"
    content: "GPT-5는 총 2조 개의 파라미터를 사용하며... 아키텍처를 혁신적으로..."
    → 키워드 ["2조", "파라미터", "아키텍처"] 중 3/3 매칭 → Pass

  섹션 2:
    subtitle: "API 가격 및 멀티모달 지원"
    content: "새로운 벤치마크에서 GPT-4 대비 30% 성능 향상... 추론 속도..."
    → 키워드 ["API", "가격", "멀티모달"] 중 0/3 매칭 → Major (주제 불일치)
```

**정합성 이슈 심각도:**
- 검증 5건 중 0건 불일치: Pass
- 검증 5건 중 1건 불일치: Minor
- 검증 5건 중 2건+ 불일치: Major
...
| sections_en 존재 | 모든 기사 | 누락 → EN 세부정보 불가 | Major |
| sections_en 구조 | 각 섹션에 subtitle + content 존재 | subtitle 또는 content 누락 | Major |
...
**주의**: EN 필드 누락은 Phase 3/4 번역 실패에 기인. `미번역 EN 기사 N개` 로그와 교차 확인. `[간이 번역 복구]` 로그가 있으면 기본 필드(제목+one_line)는 복구되었으나 sections_en/why_important_en은 여전히 누락일 수 있음.
...
| sections 2개+ | {total} | {ok} | {short}건 0-1개 | Major/Pass |
| subtitle 구체성 | {total} | {ok} | {generic}건 일반적 | Major/Pass |
| content 서술체 | {total} | {ok} | {violations}건 위반 | Major/Pass |
...
| 2 | "Meta 대규모 감축" | sections content | "감축됨" (개조식) | "감축됐다" (서술체) |
| 3 | "Google AI 신기술" | sections subtitle | "기술이 공개되었다" (문장형) | "신기술 공개 배경" (명사구) |
```

**3) 콘텐츠 품질 샘플 검증 (하이라이트 3건 + Top 2건):**

```
| # | 기사 제목 | one_line | sections | subtitle-content 정합성 | why_important | 종합 |
|---|----------|----------|----------|------------------------|---------------|------|
| 1 | "OpenAI, GPT-5..." | Pass — 사건 명확 | Pass — 3섹션, subtitle 구체적, 수치 포함 | Pass — 3/3 키워드 매칭 | Pass — 구체적 영향 | Pass |
| 2 | "Meta 감축설..." | Minor — 주어 생략 | Major — subtitle "주요 내용"(일반적) | Major — 주제 불일치 (subtitle "가격 정책" ↔ content는 성능만) | Pass — 구체적 | Major |
```

**판정 기준:**
- **Pass**: 말투 위반 0건 + 콘텐츠 이슈 0-1건 + subtitle-content 불일치 0건 + sections 부족(0-1개) 0-2건
- **Minor**: 말투 위반 1-3건 또는 콘텐츠 이슈 2-3건 또는 subtitle-content 불일치 1건
- **Major**: 말투 위반 4건+ 또는 콘텐츠 이슈 4건+ 또는 subtitle-content 불일치 2건+ 또는 sections 부족 5건+
- **Critical**: one_line 누락 5건+ 또는 말투 규칙 전면 미준수 (10건+)
- **Unverified**: 접기 섹션 없음 + Firestore 조회 불가 → 콘텐츠 품질/subtitle-content 정합성 검증 불가. 로그 통계 기반 검사(필드 완결성, 말투 자동 감지)만 수행됨. "Pass"와 명확히 구분 — 데이터 부족으로 품질 보증 불가 상태
...
| sections content 추상적 (수치·스펙 없음) | LLM이 추출 우선순위 미준수 | 프롬프트 금지 패턴 강화 + 구체적 예시 추가 (prompt-engineer) |
| sections subtitle 일반적/모호 | LLM이 구체적 명사구 규칙 미준수 | 프롬프트 subtitle 금지 단어 + 좋은/나쁜 예시 강화 (prompt-engineer) |
| sections content 개조식 혼입 | LLM이 서술체 규칙 미준수 (기존 key_points 개조식 습관) | 프롬프트 "~임/~됨 개조식 금지" 강조 + 서술체 예시 추가 (prompt-engineer) |
...
| one_line↔sections 내용 중복 | 중복 금지 규칙 미준수 | 프롬프트 중복 검증 규칙 강화 (prompt-engineer) |
| EN 필드 누락 | Phase 3/4 번역 배치 실패 | news_team.py `_process_articles()` Phase 3/4 확인 (backend-pipeline-developer) |
| sections 0-1개 다수 | 스크래핑 실패 또는 프롬프트 문제 | 스크래핑 실패율 확인 → 본문 충분한데 부족이면 프롬프트 조정 (prompt-engineer) |

**수정 대상 파일:** `scripts/agents/news_team.py`
- 요약 프롬프트: `_summarize_batch()` 내 전체 프롬프트 (one_line/sections/why_important/background 지시문)
- 말투 규칙 테이블: `_summarize_batch()` 내 `=== 말투 규칙 총정리 ===` 섹션
- sections 톤 검증: `_SEC_BAD_RE` 정규식 (sections content 경어체 위반 탐지)
...
| sections 추출 우선순위·금지 패턴 | prompt-engineer | `_summarize_batch()` sections 지시문 |
| subtitle 구체성 강화 | prompt-engineer | `_summarize_batch()` sections subtitle 지시문 |
...
- **Minor**: 허용 범위 내 이슈 (sections 2개, 소수 스크래핑 실패, L7 소량 건수)
- **Pass**: 정상 작동 확인
- **Unverified**: 데이터 부족으로 검증 불가 (요약 품질 전용 — 접기 섹션 없음 + Firestore 조회 불가 시). "Pass"와 명확히 구분되며, 리포트에 `Unverified — 데이터 미확보`로 표기
...
| 요약 말투 혼용 (서술체↔해요체) | prompt-engineer | news_team.py `_summarize_batch()` 말투 규칙 |
| 요약 콘텐츠 품질 (sections 추상적/subtitle 일반적, why_important 빈 평가, 내용 중복) | prompt-engineer | news_team.py `_summarize_batch()` 지시문 |
...
| 요약 프롬프트 (말투+콘텐츠 규칙) | news_team.py | `_summarize_batch()` — one_line/sections/why_important/background 지시문 + 말투 규칙 테이블 |
| sections 톤 검증 정규식 | news_team.py | `_SEC_BAD_RE` — sections content 경어체/개조식 위반 탐지 |
...
| 9 | 요약 품질 | Pass | 말투 위반 0건, sections 부족 0건, subtitle 구체적, subtitle-content 불일치 0건, 콘텐츠 이슈 없음 | 정상 |
...
| sections 2개+ | {total} | {ok} | {short}건 0-1개 | Pass |
| subtitle 구체성 | {total} | {ok} | {generic}건 일반적 | Pass |
| content 서술체 | {total} | {ok} | {violations}건 위반 | Pass |
...
| # | 기사 제목 | one_line | sections | subtitle-content 정합성 | why_important | 종합 |
|---|----------|----------|----------|------------------------|---------------|------|
(기사별 판정 기입 — 데이터 미확보 시 "Unverified — 접기 섹션/Firestore 미제공"으로 표기)
