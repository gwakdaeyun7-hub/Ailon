...
**수정 대상 파일:** `scripts/agents/news_team.py` — `_llm_ai_filter_batch()` 함수 (line ~664)
...
**수정 대상 파일:** `scripts/agents/news_team.py` — `_CLASSIFY_PROMPT` (line ~1341)
...
- `DEDUP_THRESHOLD` (line ~1034): 제목 유사도 임계값 (현재 0.65)
- `EMBED_DEDUP_THRESHOLD` (line ~1035): 임베딩 유사도 임계값 (현재 0.92)
- `_DEDUP_STOPWORDS` (line ~1068): 변별력 없는 토큰 set
- `_normalize_title()` (line ~1056): title=None 방어 포함
- `_extract_key_tokens()` (line ~1071): text=None 방어 포함
- `_extract_product_versions()` (line ~1113): title=None 방어 포함
- L5 조건: `names_overlap >= 3 and nums_overlap >= 1` (line ~1236)
- L7 `overlap_ratio >= 0.30` (line ~1263)
...
- `_VERIFY_PROMPT` (line ~595): 검증 프롬프트 — 이해도 검증 항목 추가
...
- `_regex_extract_verification()` (line ~648): verifier JSON 파싱 완전 실패 시 regex 폴백 — verified, confidence, principleAccuracy, insightClarity, deepDiveDepth, factCheck 필드를 정규식으로 직접 추출
...
| 컴포넌트 | 파일 | 함수/변수 | 라인 |
|----------|------|-----------|------|
| AI 필터 (EN) | news_team.py | `_llm_ai_filter_batch()` EN 프롬프트 | ~740+ |
| AI 필터 (KO) | news_team.py | `_llm_ai_filter_batch()` KO 프롬프트 | ~675+ |
| AI 필터 소스 분기 | news_team.py | `_llm_filter_sources()` — Tier 3 + NEEDS_AI_FILTER 소스 필터링 | ~787+ |
| 분류 프롬프트 | news_team.py | `_CLASSIFY_PROMPT` | ~1341+ |
| 분류 배치 | news_team.py | `_classify_batch()` | ~1502 |
| 의심 키워드 | news_team.py | `_SUSPECT_KEYWORDS` | ~1709+ |
| 중복 임계값 | news_team.py | `DEDUP_THRESHOLD`, `EMBED_DEDUP_THRESHOLD` | ~1034-1035 |
| 중복 stopwords | news_team.py | `_DEDUP_STOPWORDS` | ~1068 |
| 중복 L5 조건 | news_team.py | `_deduplicate_candidates()` L5 | ~1230+ |
| 중복 L7 조건 | news_team.py | `_deduplicate_candidates()` L7 | ~1250+ |
| 랭킹 프롬프트 | news_team.py | `_RANK_PROMPT` | ~1373+ |
| 랭킹 로직 | news_team.py | `_rank_category()` | ~1396+ |
| 랭킹 token_budget | news_team.py | `_rank_category()` 내 | ~1433 |
| 컨텍스트 축소 | news_team.py | `_rank_category()` 내 ctx_len | ~1419+ |
...
| 학문스낵 검증 프롬프트 | principle_team.py | `_VERIFY_PROMPT` | ~595 |
...
| 학문스낵 regex 폴백 | principle_team.py | `_regex_extract_verification()` | ~648 |
