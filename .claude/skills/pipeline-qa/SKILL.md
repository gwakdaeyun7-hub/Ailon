...
**v2 구조**: content_ko/content_en 자유 형식 마크다운. content_ko 존재 시 새 단일 스크롤 UI 사용. 레거시 카드 구조(foundation/application/integration/deepDive)는 content_ko 없을 때만 폴백. `scripts/curated_principles/{seed_id}.md` 파일이 있으면 LLM 생성 bypass.

로그 패턴:
- `[seed_selector] 전체 시드 풀: {n}개`
- `[seed_selector] 최근 30일 사용 시드: {n}개`
- `[curated] {seed_id} 큐레이션 콘텐츠 로드` — curated_principles/ 파일 사용 (LLM bypass, content_source='curated')
- `[content_generator] 생성 완료` — LLM 생성 (content_source='generated')
- `[verifier] 검증 결과 — verified={bool}, confidence={float}, contentQuality={float}, bilingualConsistency={float}`
- `재시도 {n}/3` — 검증 실패 후 재생성
...
| **difficulty 적합성** | beginner: 일상 비유 중심, 수식 최소. advanced: 수학적 엄밀성, 수렴 조건 등 | beginner인데 수식/전문용어 과다, advanced인데 피상적 | Major |
...
| 학문스낵 콘텐츠 추상적/모호 | prompt-engineer | principle_team.py 생성/검증 프롬프트 |
| 학문스낵 검증 임계값, 시드 다양성 | backend-pipeline-developer | principle_team.py 로직 |
| 학문스낵 curated 콘텐츠 품질 | prompt-engineer | scripts/curated_principles/*.md |
| 브리핑 프롬프트 품질 (톤, 구조, 커버리지) | prompt-engineer | generate_features.py 프롬프트 |
...
| 학문스낵 생성 로직 | principle_team.py | `content_generator()` — curated 우선, LLM 폴백 | ~499 |
