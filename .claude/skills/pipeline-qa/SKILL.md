...
- 각 카테고리 Top 5 기사 제목을 확인하여 해당 카테고리에 맞는지 판정
  ...
  - research Top 5: 논문/벤치마크/알고리즘인지? 제품 출시면 오분류
...
- **경계 사례 Examples** (프롬프트에 이미 포함됨): "CiteAudit 논문"→research, "OpenClaw 팬 미팅"→industry_business, "GeForce NOW 15종 신규 게임"→industry_business

**수정 대상 파일:** `scripts/agents/news_team.py` — `_CLASSIFY_PROMPT` (line ~1353)
...
- `DEDUP_THRESHOLD` (line ~1046): 제목 유사도 임계값 (현재 0.65)
- `EMBED_DEDUP_THRESHOLD` (line ~1047): 임베딩 유사도 임계값 (현재 0.92)
- `_DEDUP_STOPWORDS` (line ~1080): 변별력 없는 토큰 set
- `_normalize_title()` (line ~1068): title=None 방어 포함
- `_extract_key_tokens()` (line ~1083): text=None 방어 포함
- `_extract_product_versions()` (line ~1125): title=None 방어 포함
- L5 조건: `names_overlap >= 3 and nums_overlap >= 1` (line ~1248)
- L7 `overlap_ratio >= 0.30` (line ~1275)
...
**수정 대상 파일:** `scripts/agents/news_team.py`
- `_RANK_PROMPT` (line ~1373): 랭킹 기준 프롬프트
- `_rank_category()` (line ~1396): token_budget, ctx_len 임계값
- 컨텍스트 축소 기준: line ~1409-1414
- token_budget: line ~1433
...
| regex 폴백 작동 | 에러 없이 완료 | `JSON 파싱 실패 → regex 폴백으로 필드 추출 성공` 로그 → 정상 작동 확인 (단, 반복 발생 시 근본 원인 조사 필요) | Minor |
...
- `JSON 파싱 실패 → regex 폴백으로 필드 추출 성공` 로그 → `_regex_extract_verification()`이 verified, confidence, principleAccuracy, insightClarity, deepDiveDepth, factCheck 필드를 regex로 직접 추출하여 정상 검증 결과 생성. retry_reseed 경로 대신 정상 결과 사용 → 정상 작동 (단, 반복 발생 시 Gemini 응답 형식 변화 조사 필요)
...
- `_fix_unescaped_quotes()` (line ~79): json.loads 에러 위치(colno)를 사용하여 이스케이프 안 된 따옴표를 반복 수정 (최대 20회)
- `_safe_json_parse()` (line ~117-149): Phase 1 `***` strip + 즉시 json.loads + `_fix_unescaped_quotes` 폴백, `phase1_applied` 플래그로 Phase 2 스킵 제어
- `_CONTENT_PROMPT` (line ~344): 생성 프롬프트 — 설명 구체성 강화
- `_CONTENT_PROMPT` 내 deepDive 스키마 (line ~399-410): bridge/limits 구체성 강화
- `content_generator()` (line ~452): 생성 로직 — 필수 키 누락 시 `content=None` 반환 → retry_reseed
- `_VERIFY_PROMPT` (line ~549): 검증 프롬프트 — 이해도 검증 항목 추가
- `verifier()` (line ~602): 검증 로직 + confidence 임계값 — content_json[:4000] 입력, 최대 3회 파싱 시도, 빈 응답 감지, 전체 실패 시 기본 fail(verified=False, confidence=0.0) 반환
- `_regex_extract_verification()` (line ~653): verifier JSON 파싱 완전 실패 시 regex 폴백 — verified, confidence, principleAccuracy, insightClarity, deepDiveDepth, factCheck 필드를 정규식으로 직접 추출
- verifier 내 regex 폴백 호출 (line ~720): `_regex_extract_verification()` 호출하여 retry_reseed 대신 정상 검증 결과 사용
...
| bridge 보존/변형 미구분 | prompt-engineer | _CONTENT_PROMPT 내 deepDive 스키마 강화 (line ~399-410) |
...
| 컴포넌트 | 파일 | 함수/변수 | 라인 |
|----------|------|-----------|------|
| 요약 프롬프트 (tags/glossary 생성) | news_team.py | 요약 프롬프트 내 tags/glossary 지시문 | ~434-449 |
| tags/glossary 파싱 | news_team.py | `_apply_batch_results()` | ~558-575 |
| 용어 축적 | generate_features.py | `accumulate_glossary()` | ~333-415 |
...
| 컴포넌트 | 파일 | 함수/변수 | 라인 |
|----------|------|-----------|------|
| AI 필터 (EN) | news_team.py | `_llm_ai_filter_batch()` EN 프롬프트 | ~735-768 |
| AI 필터 (KO) | news_team.py | `_llm_ai_filter_batch()` KO 프롬프트 | ~670-714 |
| AI 필터 소스 분기 | news_team.py | `_llm_filter_sources()` — Tier 3 + NEEDS_AI_FILTER 소스 필터링 | ~782-868 |
| 분류 프롬프트 | news_team.py | `_CLASSIFY_PROMPT` | ~1353-1382 |
| 분류 배치 | news_team.py | `_classify_batch()` | ~1514 |
| 의심 키워드 | news_team.py | `_SUSPECT_KEYWORDS` | ~1721-1725 |
| 중복 임계값 | news_team.py | `DEDUP_THRESHOLD`, `EMBED_DEDUP_THRESHOLD` | ~1046-1047 |
| 중복 stopwords | news_team.py | `_DEDUP_STOPWORDS` | ~1080 |
| 중복 L5 조건 | news_team.py | `_deduplicate_candidates()` L5 | ~1242-1252 |
| 중복 L7 조건 | news_team.py | `_deduplicate_candidates()` L7 | ~1262-1280 |
| 랭킹 프롬프트 | news_team.py | `_RANK_PROMPT` | ~1385-1405 |
| 랭킹 로직 | news_team.py | `_rank_category()` | ~1408-1480 |
| 랭킹 token_budget | news_team.py | `_rank_category()` 내 | ~1445 |
| 컨텍스트 축소 | news_team.py | `_rank_category()` 내 ctx_len | ~1421-1426 |
| 브리핑 생성 함수 | generate_features.py | `generate_daily_briefing()` | ~183 |
| 브리핑 프롬프트 | generate_features.py | prompt in `generate_daily_briefing()` | ~214-224 |
| 브리핑 입력 기사 선정 | generate_features.py | highlights + cat_articles 조합 | ~189-202 |
| 학문스낵 생성 프롬프트 | principle_team.py | `_CONTENT_PROMPT` | ~344-449 |
| 학문스낵 Deep Dive 섹션 | principle_team.py | `_CONTENT_PROMPT` 내 인라인 JSON 스키마 | ~399-410 |
| 학문스낵 검증 프롬프트 | principle_team.py | `_VERIFY_PROMPT` | ~549 |
| 학문스낵 생성 로직 | principle_team.py | `content_generator()` | ~452 |
| 학문스낵 검증 로직 | principle_team.py | `verifier()` | ~602 |
| 학문스낵 따옴표 복구 | principle_team.py | `_fix_unescaped_quotes()` | ~79 |
| 학문스낵 JSON 파싱 | principle_team.py | `_safe_json_parse()` (Phase 1 즉시 파싱 + phase1_applied 플래그 + Phase 2 폴백) | ~117-149 |
| 학문스낵 regex 폴백 | principle_team.py | `_regex_extract_verification()` | ~653 |
| 학문스낵 verifier regex 호출 | principle_team.py | verifier 내 `_regex_extract_verification()` 호출 | ~720 |
...
- AI 필터/분류/중복/랭킹/브리핑/용어·태그/학문스낵 7개 영역을 반드시 개별 평가하여 리포트에 포함
