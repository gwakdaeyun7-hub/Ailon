...
| sub-score 재시도 | 모든 sub-score ≥0.5 | principleAccuracy/mappingAccuracy/contentQuality/bilingualConsistency 중 <0.5 → 재시도 | Major |
...
| readTime 계산 | 코드에서 글자 수 기반 자동 계산 | readTime이 LLM 출력값이면 → 코드 레그레션 (`_calc_read_time` 500자/분 기준) | Minor |
...
- `JSON 파싱 실패 → regex 폴백으로 필드 추출 성공` 로그 → `_regex_extract_verification()`이 verified, confidence, principleAccuracy, contentQuality, bilingualConsistency, factCheck, issues 필드를 regex로 직접 추출하여 정상 검증 결과 생성 (issues 배열도 보존됨). retry_reseed 경로 대신 정상 결과 사용 → 정상 작동 (단, 반복 발생 시 Gemini 응답 형식 변화 조사 필요)
...
#### 1.6.2 콘텐츠 내러티브 품질 검사 (★ 핵심)

**목적**: content_ko 자유 형식 마크다운이 "학문 원리 → AI 연결 → 실제 활용"의 내러티브 흐름을 갖추고 있는지 평가

| 검사 항목 | 좋은 예 | 나쁜 예 | 심각도 |
|-----------|---------|---------|--------|
| **학문 원리 설명** (## 섹션 1) | "금속에 열을 가하면 원자가 활발히 움직여 재배치되고, 서서히 냉각하면 결함 적은 결정 구조가 된다" — 원래 학문의 물리적/수학적 메커니즘 | "복잡한 문제에서 최적해를 찾는다" — AI 관점만, 원래 학문의 실체 없음 | Major |
| **AI 연결 내러티브** (## 섹션 2-3) | 학문 원리의 어떤 특성이 AI의 어떤 문제를 해결하는지 명시적 매핑 | "이 원리를 AI에 적용했다" — 추상적 서술만 | Major |
| **수식/알고리즘 포함** | 핵심 수식이 ``` 블록 또는 수식 패턴으로 포함, 직관적 해설 동반, ≤/≥ 등 유니코드 수학 기호 사용 | 수학적 원리인데 수식 없음, 또는 수식만 있고 해설 없음, 동일 수식이 여러 섹션에 분산 | Major |
...
| **용어 정리** | 본문에서 실제 사용된 핵심 용어만 "term - description" 패턴으로 정리. 용어 부분 40자 이내. 본문 미사용 용어 제외 | 용어 정리 없음, 본문 미사용 용어 포함, 용어 41자+ → beige 배경 미적용 | Minor |
| **마크다운 서식 일관성** | ## 제목, ``` 수식, 1. 번호 리스트, **굵은 글씨** 올바르게 사용 | 서식 혼란 (제목 누락, 코드블록 미닫힘 등) | Minor |
| **난이도 적절성** | 수식 설명에 중간 단계 포함, 전문 용어는 풀어쓴 설명 먼저 + 용어명 나중에, 구체적 사례(TSP 등) 포함, 공간적 비유(산악 지형 등) 활용, AI 연결 시 직접 적용 vs 구조적 유사성 구분 명시, 추상 개념에 직관적 보충 문장 | 지수함수 극한을 독자가 직접 추론해야 함, 전문 용어 불쑥 등장, 한 문장에 3개념+ 과밀, 이론 수치(c/ln(1+t) 등) 과다, 독립적 기원 기법을 직접 영감으로 서술, 추상 개념 도입 후 보충 설명 없음 | Major |

#### 1.6.3 콘텐츠 깊이/전문성 검사 (★ 핵심)

**목적**: content_ko가 학술적 전문성과 깊이를 보여주는지 평가 — "역사, 핵심 수학, AI 매핑, 한계까지 깊이 있게 다뤘구나"

| 검사 항목 | 좋은 예 | 나쁜 예 | 심각도 |
|-----------|---------|---------|--------|
| **학문적 기원** | "1953년 Metropolis 알고리즘에서 출발, 1983년 Kirkpatrick이 조합최적화에 적용" — 인물+시기+구체적 맥락 | "옛날에 연구자들이 발견했다" — 누가, 언제, 무엇을 불명확 | Major |
| **학문→AI 매핑** | "에너지→목적함수, 물리온도→탐색온도T" — 명시적 대응(X→Y) 최소 2쌍 + 각 쌍의 이유 | "이 원리에서 영감을 받아 AI에 적용했다" — 매핑 없이 추상적 서술만 | Critical |
| **수식 포함** | 수학/물리/정보이론/통계/공학 분야에서 ``` 블록 내 핵심 수식 + 직관적 해설 | 수학적 원리인데 수식 없음 | Major |
| **알고리즘 스텝** | 번호 리스트(1. 2. 3.)로 핵심 단계를 구조화 | 알고리즘 설명이 산문으로만 흩어져 있음 | Minor |
| **difficulty 적합성** | intermediate: 수식에 중간 단계 해설 포함, 전문 용어는 풀어쓴 설명 먼저. 이론 vs 실무 핵심만 압축 | 수식 극한 추론을 독자에게 맡김, 전문 용어 불쑥 등장, Geman 수치 비교 등 과밀 | Major |
| **구체적 사례/비유** | TSP 같은 구체 적용 사례, 에너지 경관(산악 지형) 같은 공간적 비유 포함 | "조합 최적화" 같은 추상어만 사용, 공간적 심상 없음 | Major |
| **복합 개념 분리** | Boltzmann Machine 등 한 문장에 2개념 이하, 3개념+ 시 문장 분리 | 한 문장에 "가중치=에너지 + 볼츠만 분포 + 확률적 활성화" 과밀 | Minor |
| **AI-specific 한계** | "gradient-based 대비 열위, 고차원 스케일링, distribution shift" — AI 적용 시만 드러나는 한계 | "아직 갈 길이 멀다" / 원리 자체의 일반 한계만 서술 | Major |

**수식 조건부 검사 기준:**
- discipline_key의 접두사가 `math`, `phys`, `info`, `stat`, `ee`, `opt` 중 하나이면 content_ko 내 수식(``` 블록 또는 수식 패턴) 필수
- 위 접두사가 아닌 경우(bio, neuro, psych, ling, soc 등) 수식 없어도 허용
- 수식이 있으면 해당 원리와 직결되는지 확인 (관련 없는 수식이면 오히려 감점)
...
- **Good (curated 자동)**: content_source='curated'이면 자동 Good — 수동 검수된 콘텐츠
- **Good (generated)**: 검증 통과(confidence≥0.8), contentQuality≥0.8, bilingualConsistency≥0.8, 내러티브 흐름 자연스러움, 수학적 원리이면 수식 포함
- **Acceptable**: 검증 통과(0.7≤confidence<0.8), 대부분 양호, 1-2개 추상적 표현
- **Poor**: 검증 통과했으나 학문→AI 매핑이 추상적, 또는 수학적 원리인데 수식 없음, 또는 AI-specific 한계 미서술 → 프롬프트 수정 필요
- **Failed**: 검증 미통과 3회 → 시드 풀 또는 생성 프롬프트 문제

**content_ko 내러티브 체크리스트** (분석 시 각 항목 예/아니오 판정):
1. [ ] 학문 원리의 **물리적/수학적 메커니즘**(구체적 물리량·변수·현상)을 포함하는가? AI 관점만 서술하면 불합격
2. [ ] 학문→AI **명시적 매핑(X→Y)** 최소 2쌍이 있는가? 매핑 없이 추상적 서술만이면 불합격
3. [ ] 구체적 사례/수치(벤치마크/%, 모델명, 연도)가 1개+ 포함되는가?
4. [ ] 수학/물리/정보이론/통계 원리인 경우 수식(``` 블록)이 content_ko에 포함되는가?
5. [ ] 한계 섹션이 **AI-specific 한계**를 다루는가? (원리 자체의 일반 한계만이면 불합격)
6. [ ] 전체를 읽었을 때 "학문 원리→AI 연결→실제 활용→한계"의 내러티브가 자연스러운가?
7. [ ] content_en이 content_ko의 내용을 충실히 반영하는가? (길이 70~130% 범위)
8. [ ] readTime이 코드에서 글자 수 기반으로 계산되었는가? (_calc_read_time, 500자/분)
9. [ ] 마크다운 서식(## 제목, ``` 수식, 번호리스트, **볼드**)이 SnapsContentRenderer 파싱 규칙과 호환되는가?
10. [ ] AI 연결이 직접 적용과 구조적 유사성을 구분하여 서술하는가?
11. [ ] 동일 수식이 여러 섹션에 중복 없이 한 곳에 통합되어 있는가?
...
- `verifier()` (line ~681): 검증 로직 + confidence 임계값 — content_json[:6000] 입력 (indent=None 직렬화), 최대 3회 파싱 시도, 빈 응답 감지, 전체 실패 시 기본 fail(verified=False, confidence=0.0) 반환
...
2. **부자연스러운 경우 (사실 전달에 '...' 강제 부착):**
   ...
   - 인물/기업 동향: "~사임...", "~합류...", "~인수..."
   - '주제...부가정보' 구분자 패턴: "X 공개... Y 효과", "Z 출시... W 혁신" — 가장 흔한 남용 패턴. 쉼표로 연결해야 함
...
| 이슈 유형 | 원인 | 수정 |
|-----------|------|------|
| '...' 남용 (>50% 사용률) | 프롬프트에서 "~좋음" 표현이 "~해라"로 해석됨 | 프롬프트에 "20-30%만" 명시 + 확정 사실 금지 + 구분자 금지 (2026-03-14 적용 완료) |
| 부자연스러운 '...' 반복 패턴 | LLM이 '주제...부가정보' 구분자로 '...' 사용 | 프롬프트에 나쁜 예시 추가 + 쉼표 사용 지시 (2026-03-14 적용 완료) |
...
**수정 대상 파일:** `scripts/agents/news_team.py`
- 번역+요약 프롬프트: `_summarize_batch()` 내 `title_rule` (line ~356-368)
- EN 제목 프롬프트: `en_fields_rule` 내 `display_title_en` (line ~371)
...
| 학문스낵 생성 프롬프트 | principle_team.py | `_CONTENT_PROMPT` — v2 자유 형식 마크다운 생성 | ~390+ |
| 학문스낵 curated 로드 | principle_team.py | `_load_curated_content()` — curated_principles/ 파일 로드 | ~350+ |
| 학문스낵 검증 프롬프트 | principle_team.py | `_VERIFY_PROMPT` — contentQuality/bilingualConsistency 평가 | ~589 |
| 학문스낵 생성 로직 | principle_team.py | `content_generator()` — curated 전용 (LLM 폴백은 curated 0개 비상 시만) | ~499 |
...
| 7 | 학문스낵 | Pass | confidence 0.85, content_ko 내러티브 양호, content_source=generated | 정상 |
