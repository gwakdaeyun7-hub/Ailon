...
2. **부자연스러운 경우 (확정 사실 서술어 뒤 '...'를 구분자로 사용):**
   ...
   - 금지 서술어 31개 (확정 사실 뒤 '...' 금지): 공개, 출시, 발표, 인수, 도입, 개발, 선언, 철회, 체결, 중단, 제기, 투자, 가동, 확대, 보류, 강화, 적용, 탑재, 시작, 달성, 증가, 전망, 본격화, 능가, 업그레이드, 전환, 추진, 가속화, 착수, 재편, 통합
...
| 이슈 유형 | 원인 | 수정 |
|-----------|------|------|
| 부자연스러운 '...' 구분자 패턴 반복 | LLM이 '확정 사실(공개/출시/도입 등) 뒤 ...로 부가정보 연결' 패턴 반복 | 프롬프트에 금지 서술어 31개 명시 + 2단계 우선순위(금지→적극사용) + ❌/✅ 예시 5개 (2026-03-16 적용). 코드 후처리 `_fix_title_separator()` 추가 (2026-03-17) — EN→KO 번역 제목에서 금지 서술어 + '...' 패턴을 쉼표로 자동 교정. 정규식 31개 서술어로 확장 (2026-03-19). 프롬프트 + 코드 2중 방어 |
