...
| 필수 필드 누락 | 전체 존재 | foundation/application/integration/deepDive/deepDiveHook/takeaway 중 누락 → `content=None` 반환 → retry_reseed | Critical |
| 문자 수 범위 | 각 필드별 기준 내 | headline >20자, body >120자, analogy >50자, coreIntuition 150~250자, bridge 200~300자, deepDiveHook 30~40자, takeaway 30~50자 등 | Minor |
| _en 필드 존재 | 모든 KO 필드에 EN 쌍 | _en 필드 누락 (deepDiveHook_en, takeaway_en 포함) → 바이링구얼 표시 불가 | Major |
...
| **비유 메커니즘** (foundation.analogy) | "편지에 같은 내용을 두 번 적어 한 줄이 지워져도 복구하는 것" — 핵심 메커니즘(여분 정보로 오류 복구)을 비유. 작동의 핵심 단계(조건부 수용, 피드백, 경쟁 등)가 비유에 반영되어야 함 | "두 번 말해서 확인하는 것" — 결과만 비유, 어떻게 복구하는지 메커니즘 없음 | Major |
...
| **영향 구체성** (integration.impact) | "ChatGPT의 답변 품질을 30% 향상시킨 핵심 기법" — 수치(벤치마크/%) 또는 구체적 모델/프로젝트명 1개+ 필수 | "혁신적으로 향상시켰다" / "VLSI 설계, NAS 성능 향상" — 추상적이거나 사례만 나열하고 수치 없음 | Major |
...
| **영감의 다리** (bridge) | "에너지→목적함수, 물리온도→탐색온도T. 에너지가 목적함수인 이유는 둘 다 '최소화할 대상'이므로... Boltzmann분포 보존 / 결정격자·원자간 상호작용 생략" — 명시적 매핑(X→Y) 최소 2쌍 + 각 쌍마다 WHY 1문장 + 보존/변형 구분 (200~300자) | "이 원리에서 영감을 받아 AI에 적용했다" — 매핑 없이 추상적 서술만 → deepDiveDepth ≤ 0.5 | Critical |
| **bridge 로그 출력** | bridge 내용이 로그에서 확인 가능 (70자 이상 출력) | 로그에서 bridge가 잘려서 내용 확인 불가 → content_generator 로그 확인 필요 | Minor |
| **핵심 직관** (coreIntuition) | "ΔE≤0이면 무조건 수용, ΔE>0일 때 exp(-ΔE/T)로 확률 판단. T↑→수용확률↑→광역탐색, T↓→수렴. T0↑, cooling_rate→1이면 수렴 확률↑" — difficulty별 필수 요소가 다름 (beginner: ①②, intermediate: ①②③, advanced: ①②③④). 150~250자 | "높은 산에서 공을 굴리면 골짜기를 찾는다" — 일상 비유 수준 → deepDiveDepth ≤ 0.4 | Major |
| **수식** (formula) | discipline이 수학/물리/정보이론/통계/공학이면 LaTeX 수식 포함 | 수학적 원리인데 formula가 빈 문자열 → 조건부 필수 위반 | Major |
| **AI-specific 한계** (limits) | "distribution shift 환경에서 학습된 통계적 가정이 무너지며, adversarial attack에 취약하다" — AI 적용 시만 드러나는 구체적 한계 | "아직 갈 길이 멀다" / "냉각 스케줄 설정이 어렵고 계산 비용이 높다" — 추상적 회피, 또는 원리 자체의 일반 한계만 서술 (deepDiveDepth ≤ 0.5) | Major |
...
| difficulty 적절성 | beginner: 일상 비유 중심, ①② 수준 coreIntuition. advanced: 수학적 엄밀성, ①②③④ 모두 | beginner인데 수식/전문용어 과다, advanced인데 coreIntuition이 ①② 수준 → Verifier Section E 감점 대상 | Major |
| deepDiveHook 품질 | 콘텐츠 특화 티저 (30~40자), 특정 사실/년도/메커니즘 언급 | "더 자세히 알아보기" 같은 범용 문구 → Verifier Section F 감점 | Major |
| takeaway 품질 | 핵심 인사이트 한 문장 (30~50자), headline과 차별화 | headline 단순 반복, 또는 너무 추상적 → Verifier Section F 감점 | Major |
...
- **Good**: 검증 통과(confidence≥0.8), 인사이트 이해도 양호(insightClarity≥0.8), 딥다이브 전문성 양호(deepDiveDepth≥0.8), coreIntuition이 difficulty에 맞는 수준 (150~250자), 수학적 원리이면 formula 존재, deepDiveHook이 콘텐츠 특화, takeaway가 핵심 압축
- **Acceptable**: 검증 통과(0.7≤confidence<0.8), 대부분 필드 양호, 1-2개 추상적 표현, coreIntuition이 약간 일상 비유 수준, deepDiveHook/takeaway 존재하나 품질 평범
- **Poor**: 검증 통과했으나 핵심 필드(problem, bridge, limits)가 추상적, 또는 수학적 원리인데 formula가 빈 문자열, 또는 deepDiveHook이 범용 문구, takeaway가 headline 반복 → 프롬프트 수정 필요
...
2. [ ] analogy가 핵심 메커니즘(과정)을 비유하는가? 작동의 핵심 단계(조건부 수용, 피드백, 경쟁, 선택 등)가 비유에 반영되어야 함. (결과만 비유하면 불합격)
3. [ ] application.problem이 구체적 시나리오/수치를 포함하여 "왜 어려웠는지" 전달하는가?
4. [ ] application.body가 problem의 난제를 원리의 **어떤 특성**으로 해결하는지 논리적으로 연결하는가?
5. [ ] integration.impact가 구체적 수치(벤치마크/%) 또는 모델/프로젝트명 1개+를 포함하는가? (사례만 나열하고 수치 없으면 불합격)
...
8. [ ] bridge에서 **명시적 매핑(X→Y) 최소 2쌍** + 각 쌍마다 WHY 1문장 + 보존/변형 구분이 있는가? (200~300자) 매핑 없이 추상적 서술만이면 불합격 (deepDiveDepth ≤ 0.5)
9. [ ] coreIntuition이 **difficulty에 맞는 작동 규칙**을 포함하는가? (150~250자) beginner: ①②, intermediate: ①②③, advanced: ①②③④ 필수. advanced에서 2개 이하면 불합격 (deepDiveDepth ≤ 0.5). 일상 비유 수준이면 deepDiveDepth ≤ 0.4
10. [ ] 수학/물리/정보이론/통계 원리인 경우 formula가 존재하는가?
11. [ ] limits가 AI-specific 한계를 다루는가? 원리 자체의 일반 한계만 서술하면 불합격 (deepDiveDepth ≤ 0.5). (예: "냉각 스케줄 설정이 어렵다"는 일반 한계 → 불합격. "gradient-based 대비 열위, 고차원 스케일링, distribution shift"는 AI-specific → 합격)
12. [ ] deepDiveHook이 콘텐츠에 특화된 구체적 티저인가? (30~40자) "더 자세히 알아보기" 같은 범용 문구이면 불합격. 특정 사실/년도/인물/메커니즘 언급 필수
13. [ ] takeaway가 전체 콘텐츠의 핵심 인사이트를 한 문장으로 압축하는가? (30~50자) headline 단순 반복이면 불합격
14. [ ] readTime이 코드에서 글자 수 기반으로 계산되었는가? (_calc_read_time, 500자/분)
...
| 7 | 학문스낵 | Pass | confidence 0.85, 설명 구체적, deepDiveHook/takeaway 적절 | 정상 |
